...
description:
  ...
  failures. It handles run discovery automaticallyâ€”simply provide the branch name.
---
...
- **Behavior**: When this workflow is triggered, the agent will monitor the CI
  ...
  commands (marked with ðŸš€) as soon as a failure is detected.
...
- **Step 2 (Extract)**: Extract suggested \`npm test\` or \`npm run lint\` commands
  from the output (marked with ðŸš€).
