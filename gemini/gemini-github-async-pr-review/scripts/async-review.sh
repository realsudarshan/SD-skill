...
if [[ -z "${base_dir}" ]]; then
  echo "âŒ Must be run from within a git repository."
  exit 1
fi
...
echo "ðŸ§¹ Cleaning up previous worktree if it exists..." | tee -a "${log_dir}/setup.log"
...
echo "  â†³ [3/5] Starting Gemini code review..."
...
echo "  â†³ [4/5] Starting automated tests (waiting for build and lint)..."
...
echo "  â†³ [5/5] Starting Gemini test execution (waiting for build and lint)..."
...
while [[ "${all_done}" -eq 0 ]]; do
  clear
  echo "=================================================="
  echo "ðŸš€ Async PR Review Status for PR #${pr_number}"
  echo "=================================================="
  echo ""
  ...
  for i in "${!tasks[@]}"; do
    ...
    if [[ -f "${log_dir}/${t}.exit" ]]; then
      ...
      if [[ "${task_exit}" == "0" ]]; then
        ...
        echo "  âŒ ${t}: FAILED (exit code ${task_exit})"
      fi
      task_done[${t}]=1
    else
      echo "  â³ ${t}: RUNNING"
  ...
  echo ""
  echo "=================================================="
  echo "ðŸ“ Live Logs (Last 5 lines of running tasks)"
  echo "=================================================="
...
echo "â³ Tasks complete! Synthesizing final assessment..."
if ! "${GEMINI_CMD}" --policy "${POLICY_PATH}" -p "Read the review at ${log_dir}/review.md, the automated test logs at ${log_dir}/npm-test.log, and the manual test execution logs at ${log_dir}/test-execution.log. Summarize the results, state whether the build and tests passed based on ${log_dir}/build-and-lint.exit and ${log_dir}/npm-test.exit, and give a final recommendation for PR ${pr_number}." > "${log_dir}/final-assessment.md" 2>&1; then
  ...
  echo "âŒ Final assessment synthesis failed!"
...
echo "âœ… Final assessment complete! Check ${log_dir}/final-assessment.md"
