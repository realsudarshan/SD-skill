...
if [[ ! -d "${log_dir}" ]]; then
  ...
  echo "âŒ No logs found for PR #${pr_number} in ${log_dir}"
  exit 0
fi
...
for task_info in "${tasks[@]}"; do
  ...
  if [[ -f "${exit_file}" ]]; then
    ...
    if [[ "${exit_code}" == "0" ]]; then
      echo "âœ… ${task_name}: SUCCESS"
    else
      echo "âŒ ${task_name}: FAILED (exit code ${exit_code})"
      echo "   Last lines of ${file_path}:"
      tail -n 3 "${file_path}" | sed 's/^/      /' || true
    fi
  elif [[ -f "${file_path}" ]]; then
    echo "â³ ${task_name}: RUNNING"
    all_done=false
  else
    echo "âž– ${task_name}: NOT STARTED"
    all_done=false
  fi
done
