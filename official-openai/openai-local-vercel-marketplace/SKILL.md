---
name: marketplace
description: Vercel Marketplace expert guidance â€” discovering, installing, and building integrations, auto-provisioned environment variables, unified billing, and the vercel integration CLI. Use when consuming third-party services, building custom integrations, or managing marketplace resources on Vercel.
metadata:
  priority: 3
  docs:
...
You are an expert in the Vercel Marketplace â€” the integration platform that connects third-party services to Vercel projects with unified billing, auto-provisioned environment variables, and one-click setup.
...
Use `--framework <fw>` as the default discovery flow when framework-specific setup matters. The guide returns structured setup steps including required environment variables, SDK packages, and code snippets â€” ideal for agentic workflows.
...
**IMPORTANT: Provisioning delay after install.** After installing a database integration (especially Neon), the resource may take **1â€“3 minutes** to fully provision. During this window, connection attempts return HTTP 500 errors. Do NOT debug the connection string or code â€” just wait and retry. If local env sync was disabled or skipped, run `vercel env pull .env.local --yes` after a brief wait to get the finalized credentials.
...
# POSTGRES_URL          â€” connection string
...
No manual `.env` file management is needed â€” the variables are injected into all environments (Development, Preview, Production) automatically.

### Using Provisioned Resources

```ts
// app/api/users/route.ts â€” using Neon auto-provisioned env vars
...
```ts
// app/api/cache/route.ts â€” using Upstash auto-provisioned env vars
...
1. **Integration manifest** â€” declares capabilities, required scopes, and UI surfaces
2. **Webhook handlers** â€” respond to Vercel lifecycle events
3. **UI components** â€” optional dashboard panels rendered within Vercel
4. **Resource provisioning** â€” create and manage resources for users
...
| **Traces**         | Native drain (OpenTelemetry-compatible)               | Same â€” auto-configured on install                                                                                      |
...
> **Key distinction:** When you install an observability vendor via the Marketplace, it auto-configures drains for **logs and traces** only. Speed Insights and Web Analytics data require a separate, manually configured drain pointing to a custom endpoint. See `â¤³ skill: observability` for drain setup details.
...
```bash
# Install â€” auto-provisions env vars and creates log/trace drains
...
Check the response for a drain pointing to the vendor's ingestion endpoint. If no drain appears, the integration may need manual drain setup â€” see `â¤³ skill: observability` for REST API drain creation.
...
> **For drain payload formats and signature verification**, see `â¤³ skill: observability` â€” the Drains section covers JSON/NDJSON schemas and `x-vercel-signature` HMAC-SHA1 verification.
...
> **Payload schema reference:** See `â¤³ skill: observability` for Web Analytics drain payload formats (JSON array of `{type, url, referrer, timestamp, geo, device}` events).
...
- **Drain configuration, payload formats, signature verification** â†’ `â¤³ skill: observability`
- **Drains REST API endpoints** â†’ `â¤³ skill: vercel-api`
- **CLI log streaming (`--follow`, `--since`, `--level`)** â†’ `â¤³ skill: vercel-cli`
- **Safe project setup sequencing (link, env pull, then run db/dev)** â†’ `â¤³ skill:bootstrap`
- **Headless CMS integrations (Sanity, Contentful)** â†’ `â¤³ skill:cms`
