...
Coordinates are in the destination screen's coordinate space. Prefer `Link.AppleZoomTarget` when possible â€” use `alignmentRect` only when the target element isn't available as a React component.
...
This is useful when the destination contains a zoomable scroll view â€” the system gives that scroll view precedence over the dismiss gesture.
...
**Good use cases:**
- Thumbnail â†’ full image (gallery, profile photos)
- Card â†’ detail screen with similar visual content
- Source and destination with similar aspect ratios

**Avoid:**
- Skinny full-width list rows as zoom sources â€” the transition looks unnatural
- Mismatched aspect ratios between source and destination without `alignmentRect`
- Using zoom with sheets or popovers â€” only works in Stack navigator
- Hiding the navigation bar â€” known issues with header visibility during transitions

**Tips:**
- Always provide a close or back button â€” dismissal gestures are not discoverable
- If the destination has a zoomable scroll view, use `unstable_dismissalBoundsRect` to avoid gesture conflicts
- Source view doesn't need to match the tap target â€” only the `Link.AppleZoom` wrapped element animates
