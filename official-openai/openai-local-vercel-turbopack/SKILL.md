...
You are an expert in Turbopack â€” the Rust-powered JavaScript/TypeScript bundler built by Vercel. It is the default bundler in Next.js 16.

## Key Features

- **Instant HMR**: Hot Module Replacement that doesn't degrade with app size
- **File System Caching (Stable)**: Dev server artifacts cached on disk between restarts â€” up to 14x faster startup on large projects. Enabled by default in Next.js 16.1+, no config needed. Build caching planned next.
...
Install `sass` and import `.scss` files directly â€” Turbopack compiles them natively:
...
- **CSS ordering differs from webpack**: Turbopack may load CSS chunks in a different order. Avoid relying on source-order specificity across files â€” use more specific selectors or CSS Modules.
- **`@import` in global CSS**: Use standard CSS `@import` â€” Turbopack resolves them, but circular imports cause build failures.
...
- **ES module exports**: Only used exports are included â€” write `export` on each function/constant rather than barrel `export *`
...
- **Dynamic imports**: `import()` expressions create async chunk boundaries â€” Turbopack splits these into separate chunks automatically
...
|----------------|---------------------|
| `css-loader` + `style-loader` | Built-in CSS support â€” remove loaders |
| `sass-loader` | Built-in â€” install `sass` package |
| `postcss-loader` | Built-in â€” reads `postcss.config.js` |
...
File an issue at [github.com/vercel/next.js](https://github.com/vercel/next.js) â€” the Turbopack team tracks loader parity requests.

## Production Build Diagnostics

### Build failing with Turbopack

1. **Check for unsupported config**: Remove any `webpack()` function from next.config â€” it's ignored by Turbopack and may mask the real config
2. **Verify `turbopack.rules`**: Ensure custom rules reference valid loaders that are installed
3. **Check for Node.js built-in usage in edge/client**: Turbopack enforces environment boundaries â€” `fs`, `path`, etc. cannot be imported in client or edge bundles
...
- Audit `"use client"` directives â€” each client component boundary creates a new chunk
...
This writes a `trace.json` to the project root â€” open it in `chrome://tracing` or [Perfetto](https://ui.perfetto.dev/) to see module-level timing.
...
4. **CSS ordering changes**: Test visual regressions when migrating â€” CSS chunk order may differ.
5. **Environment boundary errors**: Server-only modules imported in client components fail at build time â€” use `server-only` package.
