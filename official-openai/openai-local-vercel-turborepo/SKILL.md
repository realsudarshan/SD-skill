...
You are an expert in Turborepo v2.8 â€” "the build system for agentic coding" â€” a high-performance build system for JavaScript/TypeScript monorepos, built by Vercel with a Rust-powered core.

## Key Features

- **Task caching**: Content-aware hashing â€” only rebuilds when files actually change
...
- `dependsOn: ["^build"]` â€” Run `build` in dependencies first (`^` = topological)
- `dependsOn: ["build"]` â€” Run `build` in the same package first (no `^`)
- `outputs` â€” Files to cache (build artifacts)
- `inputs` â€” Files that affect the task hash (default: all non-gitignored files)
- `env` â€” Environment variables that affect the task hash
- `cache: false` â€” Skip caching (for dev servers, codegen)
- `persistent: true` â€” Long-running tasks (dev servers)
- `globalDependencies` â€” Files that invalidate all task caches when changed
- `globalEnv` â€” Env vars that invalidate all task caches when changed
...
### GitHub Actions â€” parallel jobs per package
...
Run tasks in watch mode for development â€” re-executes when source files change:
...
Watch mode respects the task graph â€” if `test` depends on `build`, changing a source file re-runs `build` first, then `test`.
...
### Dry run â€” see what would execute
...
> **Note**: `turbo docs` output is optimized for AI coding agents â€” markdown format preserves context windows. The docs site also includes sample prompts for common tasks you can copy directly into your agent.
...
Turborepo is the recommended orchestration layer for Vercel's Microfrontends architecture â€” composing multiple independently-deployed apps behind a single URL.
...
â”œâ”€â”€ package.json
â”œâ”€â”€ apps/
â”‚   â”œâ”€â”€ shell/          # Layout / shell app (owns top-level routing)
â”‚   â”œâ”€â”€ dashboard/      # Micro-app: dashboard features
â”‚   â”œâ”€â”€ settings/       # Micro-app: settings features
â”‚   â””â”€â”€ marketing/      # Micro-app: public marketing site
â””â”€â”€ packages/
    â”œâ”€â”€ ui/             # Shared component library
    â”œâ”€â”€ auth/           # Shared auth utilities
    â””â”€â”€ config/         # Shared tsconfig, eslint
```
...
|----------|-------------|
| Multiple teams owning independent features | Yes â€” independent deploys + shared packages |
| Single team, single app | No â€” standard Next.js is simpler |
| Shared component library across apps | Yes â€” `packages/ui` with boundary rules |
| Gradual migration from monolith | Yes â€” extract features into micro-apps incrementally |
| Need version-skew protection | Yes â€” isolated builds per micro-app |
...
- **Granular cache invalidation**: Turborepo parses `bun.lock` to detect which specific packages changed and only invalidates caches for affected tasks â€” not the entire monorepo.
...
|----------|----------------|
| Single Next.js app | No â€” Turbopack handles bundling |
| Multiple apps sharing code | Yes â€” orchestrate builds |
| Shared component library | Yes â€” manage dependencies |
| CI taking too long | Yes â€” caching + affected |
| Team sharing build artifacts | Yes â€” remote caching |
| Enforcing architecture boundaries | Yes â€” boundary rules |
| Complex multi-step CI pipelines | Yes â€” task graph + matrix |
