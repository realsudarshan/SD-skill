...
Objectives
1.	Project overview
	â€¢	Identify the primary programming languages, frameworks, and build system.
	â€¢	Summarize the projectâ€™s core purpose and high-level architecture.
	â€¢	Describe major components, services, or modules and how they interact.
2.	Security posture and entry points
	â€¢	Identify likely user entry points and trust boundaries.
	â€¢	Describe existing security layers (e.g., authentication, authorization, validation, sandboxing, isolation, privilege boundaries).
	â€¢	Call out security-critical components and assumptions that must hold for the system to remain secure.
...
Structure the summary so an application security engineer can quickly answer questions such as:
	â€¢	Where does user input originate?
	â€¢	How is untrusted data parsed, validated, and handled?
	â€¢	What security assumptions should not be violated?
	â€¢	Where are the most likely choke points for security bugs?

Adapt the analysis to the project type. For example:
	â€¢	Web applications: where requests enter, how user data is parsed, routed, authenticated, and stored.
	â€¢	Command-line tools: supported inputs (arguments, files, environment variables, stdin) and how they are processed.
	â€¢	Network daemons: exposed ports, supported protocols, message formats, and request handling paths.
	â€¢	Operating system or low-level components: common vulnerability classes (e.g., memory corruption, logic flaws) that could lead to LPE or RCE.
...
Represent the system as a sequence of arrow-style bullets (e.g., Internet â†’ API Server, User Input -> Application Logic, etc). For each boundary, document:
	â€¢	the primary data types crossing the boundary,
	â€¢	the communication channel or protocol,
	â€¢	the security guarantees (e.g., authentication, origin checks, encryption, rate limiting), and
	â€¢	any input validation, normalization, or schema enforcement performed.
...
- Include 1â€“2 repo-path anchors per major claim; do not dump every match.
