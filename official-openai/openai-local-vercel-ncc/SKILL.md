---
name: ncc
description: 'Expert guidance for @vercel/ncc â€” a simple CLI for compiling Node.js modules into a single file with all dependencies included. Use when bundling serverless functions, CLI tools, or any Node.js project into a self-contained file.'
metadata:
  priority: 4
  docs:
...
# @vercel/ncc â€” Node.js Compiler Collection
...
- **Serverless functions** â€” deploy a single file instead of `node_modules`
- **CLI tools** â€” distribute a self-contained executable
- **Docker images** â€” reduce image size by eliminating `node_modules`
...
| `-w, --watch` | Watch mode â€” rebuild on changes |
...
ncc natively supports TypeScript â€” no separate `tsc` step needed:
...
GitHub Actions require all dependencies bundled â€” ncc is the recommended bundler for custom JS/TS actions.

## Key Points

1. **Single-file output** â€” all dependencies inlined, no `node_modules` needed at runtime
2. **TypeScript native** â€” compiles `.ts` files directly using the project's `tsconfig.json`
3. **No config file** â€” entirely driven by CLI flags
4. **Asset handling** â€” non-JS files are automatically copied to the output directory
5. **Use externals for native modules** â€” binary `.node` modules often need to be external
6. **Source maps for debugging** â€” use `-s` flag to generate `.js.map` files
7. **Watch mode for dev** â€” use `-w` for fast iteration during development

## Official Resources

- [ncc GitHub](https://github.com/vercel/ncc)
- [Vercel Blog â€” ncc Introduction](https://github.com/vercel/ncc)
