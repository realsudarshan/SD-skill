...
description: "AI generation persistence patterns â€” unique IDs, addressable URLs, database storage, and cost tracking for every LLM generation"
metadata:
  priority: 6
  docs:
...
Every call to an LLM costs real money and produces unique output that cannot be exactly reproduced. Treat generations like database records â€” assign an ID, persist immediately, and make them retrievable.

## Core Rules

1. **Generate an ID before the LLM call** â€” use `nanoid()` or `createId()` from `@paralleldrive/cuid2`
2. **Persist every generation** â€” text and metadata to database, images and files to Vercel Blob
3. **Make every generation addressable** â€” URL pattern: `/chat/[id]`, `/generate/[id]`, `/image/[id]`
4. **Track metadata** â€” model name, token usage, estimated cost, timestamp, user ID
5. **Never stream without saving** â€” if the user refreshes, the generation must survive
...
export async function POST(req: Request) {
  ...
  // Redirect to the generation page â€” it handles streaming
  redirect(`/chat/${id}`);
}
```
...
- **Streaming to client without saving** â€” generation lost on page refresh. Always write to DB as tokens arrive or on completion.
- **Routes without `[id]` segments** â€” `/api/chat` with no ID means generations aren't addressable. Use `/chat/[id]`.
- **Re-generating identical prompts** â€” check cache first. Same prompt + same model = same cost for no new value.
- **Ephemeral base64 images** â€” generated images served inline are lost when the component unmounts. Save to Vercel Blob.
- **Missing metadata** â€” always store model name, token counts, and timestamp. You need this for cost tracking and debugging.
- **Client-only state** â€” storing generations only in React state or localStorage. Use a database â€” generations must survive across devices and sessions.
