...
You are an expert in Vercel environment variable management â€” `.env` file conventions, the `vercel env` CLI, OIDC token lifecycle, and environment-specific configuration.
...
- **Never commit secrets** to `.env`, `.env.development`, or `.env.production` â€” use `.local` variants or Vercel environment variables
- `.env.local` is always gitignored by Next.js â€” this is where `vercel env pull` writes secrets
- Variables prefixed with `NEXT_PUBLIC_` are exposed to the browser bundle â€” never put secrets in `NEXT_PUBLIC_` vars
...
```bash
# Interactive â€” prompts for value and environments
...
1. **On Vercel deployments**: `VERCEL_OIDC_TOKEN` is automatically injected as a short-lived JWT and auto-refreshed â€” zero configuration needed
...
// AI Gateway uses OIDC by default â€” no manual token handling needed
...
- **Production** â€” only `vercel.app` production deployments
- **Preview** â€” branch/PR deployments
- **Development** â€” `vercel dev` and `vercel env pull`
...
`vercel env pull .env.local` **replaces the entire file** â€” any manually added variables (custom secrets, local overrides, debug flags) are lost. Always back up or re-add custom vars after pulling:
...
1. **Use `vercel env pull` as part of your setup workflow** â€” document it in your README
2. **Never hardcode secrets** â€” always use environment variables
3. **Scope narrowly** â€” don't give preview deployments production database access
4. **Rotate OIDC tokens regularly in local dev** â€” re-pull when you see auth errors
5. **Use `.env.example`** â€” commit a template with empty values so teammates know which vars are needed
6. **Prefix client-side vars with `NEXT_PUBLIC_`** â€” and never put secrets in them
7. **Keep custom vars in `.env.development.local`** â€” protects them from `vercel env pull` overwrites
