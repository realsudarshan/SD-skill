...
metadata:
  ...
  promptSignals:
    phrases:
      ...
      - "use workflow"
      - "use step"
      # Pipeline / multi-step language (the BIG gap â€” natural product prompts)
      - "multi-step pipeline"
      - "multi step pipeline"
      - "multi-step process"
      - "multi step process"
...
**Security**: Upgrade to `workflow@>=4.2.0-beta.64` â€” versions â‰¤4.1.0-beta.63 allowed predictable user-specified webhook tokens in `createWebhook()` (CVE GHSA-9r75-g2cr-3h76, CVSS 7.5). Run `npx workflow@latest` to update.
...
Do NOT use the `src/` directory with WDK projects. The `@` alias must resolve `@/workflows/...` correctly â€” this only works when `workflows/` and `app/` are at the same level.
...
```
workflows/
  my-workflow.ts              â† workflow definition ("use workflow" + "use step")
app/api/
  my-workflow/route.ts        â† POST handler: start(workflow, args) â†’ { runId }
  readable/[runId]/route.ts   â† GET handler: SSE stream from run.getReadable()
  run/[runId]/route.ts        â† GET handler: run status via getRun(runId)
```

tsconfig.json paths: `"@/*": ["./*"]` â€” `@/workflows/my-workflow` resolves to `./workflows/my-workflow`.
...
tsconfig.json paths: `"@/*": ["./src/*"]` â€” `@/workflows/my-workflow` resolves to `./src/workflows/my-workflow`.

**Never** use `@/../workflows/` or `@/../../workflows/` â€” these are broken import paths that will fail at build time.
...
async function stepOne(input: string): Promise<string> {
  ...
  try {
    ...
    // Full Node.js access here â€” fetch, db calls, etc.
...
**IMPORTANT**: Never call the workflow function directly. Always use `start()` from `"workflow/api"` â€” it registers the run, creates the execution context, and returns a `{ runId }`.
...
**Rendering workflow events in the UI**: When workflow events contain AI-generated text (narratives, briefings, reports), render them with `<MessageResponse>` from `@/components/ai-elements/message` â€” never as raw `{event.content}`. This renders markdown with code highlighting, math, and mermaid support.
...
## Hooks â€” Waiting for External Events
...
// Define at module scope â€” reusable across workflows
...
export async function approvalGate(orderId: string): Promise<{ status: string }> {
  ...
  // .create() returns a hook instance â€” NOT directly callable
...
### 2. Resume route (API file â€” required!)
...
**You MUST create this route.** Without it, the workflow suspends forever â€” the client has no way to resume it.
...
async function searchDatabase(query: string) {
  "use step";
  // Full Node.js access â€” real DB calls here
...
**Every step function MUST have `console.log` at entry and exit.** This is the single most important debugging practice â€” without it, you cannot tell which step is hanging.
...
**Workflow-level logging** â€” log at every orchestration point:
...
```bash
# List recent runs â€” look for "running" status that's been active too long
...
# Or check the Vercel dashboard: Project â†’ Deployments â†’ Functions tab
```

Look for:
- **Missing step entry logs** â€” the step before the missing one is where execution stopped
- **Timeout errors** â€” Vercel function timeout (default 60s hobby, 300s pro)
- **OIDC/credential errors** â€” `gateway()` calls fail silently without `vercel env pull`
- **Memory errors** â€” large payloads in steps can OOM the function
...
If the workflow powers a UI, use `agent-browser` to check the frontend while inspecting backend logs â€” a hanging page often means a stuck workflow step. Check the browser console for failed fetch calls to your workflow API routes.
