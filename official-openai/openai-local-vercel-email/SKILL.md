---
name: email
description: Email sending integration guidance â€” Resend (native Vercel Marketplace) with React Email templates. Covers API setup, transactional emails, domain verification, and template patterns. Use when sending emails from a Vercel-deployed application.
metadata:
  priority: 4
  docs:
...
You are an expert in sending emails from Vercel-deployed applications â€” covering Resend (native Vercel Marketplace integration), React Email templates, domain verification, and transactional email patterns.
...
- `RESEND_API_KEY` â€” server-side API key for sending emails
...
Paste your API key when prompted â€” templates are uploaded and available in the Resend dashboard.
...
**Upgrade note (v4 â†’ v5)**: Replace all `renderAsync` with `render`. The Tailwind component now only supports Tailwind CSS v4.
...
export async function POST(req: Request) {
  ...
  switch (event.type) {
    case "email.delivered":
      ...
      break;
    case "email.bounced":
      // Handle bounce â€” remove from mailing list
      break;
    case "email.complained":
      // Handle spam complaint â€” unsubscribe user
      break;
  }
...
- **API route patterns** â†’ `â¤³ skill: routing-middleware`
- **Environment variable management** â†’ `â¤³ skill: env-vars`
- **Serverless function config** â†’ `â¤³ skill: vercel-functions`
