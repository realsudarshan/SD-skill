...
You are an expert in the Vercel Firewall â€” a multi-layered security solution with automatic DDoS protection, a customizable Web Application Firewall (WAF), bot management, and rate limiting.
...
No configuration needed â€” DDoS protection is always active.
...
### Bot Protection (GA â€” Free on All Plans)

Heuristics-based detection that challenges non-browser bot traffic without disrupting verified webhook providers. Formerly "Bot Filter" during beta â€” renamed to Bot Protection at GA. Enable in log-only mode first to preview traffic impact:
...
- Configured via dashboard only: open `https://vercel.com/{team}/{project}/firewall` â†’ **Bot Management** â†’ **Attack Challenge Mode**
...
| OWASP CRS | â€” | â€” | Yes |
| Token Bucket algo | â€” | â€” | Yes |
| Custom rate limit keys | â€” | â€” | Yes |

## Observability

- Security event logs in the Firewall tab
- **IP enrichment** â€” hover any IP in the Firewall dashboard to see ASN, location, and metadata
