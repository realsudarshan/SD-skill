...
Choose one charge type per integration â€” do not mix them. For most platforms, start with destination charges:
...
**Traps to avoid:** Do not use the Charges API for Connect fund flows â€” use PaymentIntents or Checkout Sessions with `transfer_data` or `on_behalf_of`. Do not mix charge types within a single integration.
