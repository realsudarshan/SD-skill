---
name: vercel-sandbox
description: Vercel Sandbox guidance â€” ephemeral Firecracker microVMs for running untrusted code safely. Supports AI agents, code generation, and experimentation. Use when executing user-generated or AI-generated code in isolation.
metadata:
  priority: 4
  docs:
...
- **Filesystem**: Ephemeral â€” artifacts must be exported before sandbox stops
...
Egress firewall uses **SNI filtering** on TLS client-hello â€” outbound connections are matched at the handshake and unauthorized destinations are rejected before data transmits. For non-TLS traffic, IP/CIDR rules are also supported.

Policies can be updated at runtime without restarting the sandbox process, enabling multi-step workflows (e.g., open access during setup â†’ deny-all before running untrusted code).
...
2. **Snapshot-based fast restart**: Install deps once â†’ snapshot â†’ create from snapshot (skip setup)
3. **Network isolation**: Allow all during setup â†’ `deny-all` before untrusted code
...
6. **File I/O workflow**: `writeFiles()` â†’ execute â†’ `readFileToBuffer()` results
...
- Production workloads â†’ use Vercel Functions
- Long-running services â†’ use a dedicated server
- Simple function execution â†’ use Serverless Functions

## References

- ðŸ“– docs: https://vercel.com/docs/vercel-sandbox
- ðŸ“– SDK reference: https://vercel.com/docs/vercel-sandbox/sdk-reference
- ðŸ“– GitHub: https://github.com/vercel/sandbox
