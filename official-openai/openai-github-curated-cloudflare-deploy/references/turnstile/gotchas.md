...
### âŒ Skipping Server-Side Validation
...
window.turnstile.render('#container', {
  sitekey: 'YOUR_SITE_KEY',
  callback: (token) => console.log('âœ“ Token:', token),
  'error-callback': (code) => console.error('âœ— Error:', code),
  'expired-callback': () => console.warn('â± Expired'),
  'timeout-callback': () => console.warn('â± Timeout')
});
```
