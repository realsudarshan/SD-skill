...
```typescript
// âŒ Wrong - lost on eviction
setTimeout(() => this.cleanup(), 3600000);

// âœ… Right - survives eviction
...
```typescript
// âŒ Wrong - expensive on every wake
...
```typescript
// âŒ Wrong - race condition
async incrementCounter() {
  const count = await this.ctx.storage.get("count") || 0;
  // âš ï¸ Another request could execute here during await
