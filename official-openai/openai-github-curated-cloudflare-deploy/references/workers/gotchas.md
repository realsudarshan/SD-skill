...
**When to use sessions:** Write â†’ Read patterns, transactions requiring consistency
...
// âœ… RPC pattern (type-safe, no serialization overhead)
export class MyDO {
  async increment() { return ++this.value; }
}
const stub = env.DO.get(id);
