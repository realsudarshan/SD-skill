...
```
Worker â†’ Edge (setup) â†’ Pool (near DB) â†’ Origin
         â†“ cached reads
         Cache
```
...
âœ… Global access to single-region DBs, high read ratios, popular queries, connection-heavy loads
âŒ Write-heavy, real-time data (<1s), single-region apps close to DB

**ðŸ’¡ Pair with Smart Placement** for Workers making multiple queries - executes near DB to minimize latency.
