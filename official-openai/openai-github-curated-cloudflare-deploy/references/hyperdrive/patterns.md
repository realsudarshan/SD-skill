...
**Benefits:** Edge setup + DB pooling = global â†’ single-region DB without replication.
...
**Benefits:** Worker executes near DB â†’ reduces latency for each query. Without Smart Placement, each query round-trips from edge.
...
// âŒ Across queries (may get different connection)
await client.query("SET work_mem = '256MB'");
...
**Best practices:**
```typescript
// âŒ Long transactions block pooling
await client.query("BEGIN");
...
// âœ… SET LOCAL within transaction
await client.query("BEGIN");
...
// âŒ Not cacheable (volatile NOW())
await sql`SELECT * FROM logs WHERE created_at > NOW()`;

// âœ… Cacheable (parameterized timestamp)
const ts = Date.now();
