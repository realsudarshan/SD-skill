...
- `export default { fetch(req, env) {} }` â†’ `export function onRequest(ctx) { const { request, env } = ctx; }`
- Use `_worker.js` for complex routing: `env.ASSETS.fetch(request)` for static files

**Other platforms â†’ Pages:**
- File-based routing: `/functions/api/users.js` â†’ `/api/users`
