...
Common problems â†’ causes â†’ solutions.
...
**Solution:** Create new token â†’ test in staging â†’ update prod â†’ monitor 24h â†’ revoke old.
...
**Solution:** Only add nullable columns, compatible type widening (intâ†’long, floatâ†’double).
...
1. âœ… **Catalog enabled:** `npx wrangler r2 bucket catalog status <bucket>`
2. âœ… **Token permissions:** Both R2 Data Catalog + R2 Storage in dashboard
3. âœ… **Connection test:** `catalog.list_namespaces()` succeeds
4. âœ… **URI format:** HTTPS, includes `/iceberg/`, correct bucket name
5. âœ… **Warehouse name:** Matches bucket name exactly
6. âœ… **Namespace exists:** Create before `create_table()`
7. âœ… **Enable debug logging:** `logging.basicConfig(level=logging.DEBUG)`
8. âœ… **PyIceberg version:** `pip install --upgrade pyiceberg` (â‰¥0.5.0)
9. âœ… **File health:** Compact if >1000 files or avg <10MB
10. âœ… **Snapshot count:** Expire if >100 snapshots
