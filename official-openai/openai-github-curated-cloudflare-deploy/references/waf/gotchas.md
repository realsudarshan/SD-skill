...
```typescript
// Common mistakes
'http.request.path' â†’ 'http.request.uri.path' // Correct field
'ip.geoip.country eq US' â†’ 'ip.geoip.country eq "US"' // Quote strings
'http.user_agent eq "Mozilla"' â†’ 'lower(http.user_agent) contains "mozilla"' // Case sensitivity
'matches ".*[.jpg"' â†’ 'matches ".*\\.jpg$"' // Valid regex
```
...
```typescript
// Error: "Invalid phase" â†’ Use exact phase name
phase: 'http_request_firewall_custom'

// Error: "Ruleset already exists" â†’ Use update() or list first
...
// Error: "Action not supported" â†’ Check phase/action compatibility
// 'execute' only in http_request_firewall_managed
// Rate limit config only in http_ratelimit phase

// Error: "Expression parse error" â†’ Common fixes:
