...
â”‚ CRITICAL: Workers Cache API â‰  Cache Reserve                   â”‚
â”‚                                                                â”‚
â”‚ â€¢ Workers caches.default / cache.put() â†’ edge cache ONLY      â”‚
â”‚ â€¢ Cache Reserve â†’ zone-level setting, automatic, no per-req   â”‚
â”‚ â€¢ You CANNOT selectively write to Cache Reserve from Workers  â”‚
â”‚ â€¢ Cache Reserve works with standard fetch(), not cache.put()  â”‚
...
```typescript
// âŒ WRONG: cache.put() bypasses Cache Reserve
...
// âœ… CORRECT: Use standard fetch for Cache Reserve compatibility
return await fetch(request);

// âœ… CORRECT: Use Cache API only for custom cache namespaces
...
**Process**: Disable Cache Reserve â†’ Call clear endpoint â†’ Wait up to 24hr â†’ Re-enable
