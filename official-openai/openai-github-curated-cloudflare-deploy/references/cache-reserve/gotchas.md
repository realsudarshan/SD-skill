...
**Solution:**
- **What is O2O**: Zone A (proxied) â†’ Zone B (proxied), both on Cloudflare
...
1. Is Cache Reserve enabled for zone?
   â†’ No: Enable via Dashboard or API
   â†’ Yes: Continue to step 2

2. Is Tiered Cache enabled?
   â†’ No: Enable Tiered Cache (required!)
   â†’ Yes: Continue to step 3

3. Does asset have TTL â‰¥ 10 hours?
   â†’ No: Increase via Cache Rules (edge_ttl override)
   â†’ Yes: Continue to step 4

4. Is Content-Length header present?
   â†’ No: Fix origin to include Content-Length
   â†’ Yes: Continue to step 5

5. Is Set-Cookie header present?
   â†’ Yes: Remove Set-Cookie or scope appropriately
   â†’ No: Continue to step 6

6. Is Vary header set to *?
   â†’ Yes: Change to specific value (e.g., Accept-Encoding)
   â†’ No: Continue to step 7

7. Is this a range request?
   â†’ Yes: Range requests bypass Cache Reserve (not supported)
   â†’ No: Continue to step 8

8. Is this an O2O (Orange-to-Orange) request?
   â†’ Yes: O2O bypasses Cache Reserve
   â†’ No: Continue to step 9

9. Check Logpush CacheReserveUsed field
   â†’ Filter logs to see if assets ever hit Cache Reserve
   â†’ Verify cf-cache-status header (should be HIT after first request)
```
