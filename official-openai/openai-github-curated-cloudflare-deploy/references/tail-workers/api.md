...
```typescript
// âœ… CORRECT - use directly with Date
const date = new Date(event.eventTimestamp);

// âŒ WRONG - don't multiply by 1000
...
- **Hex IDs:** 32+ hex digits â†’ `"REDACTED"`
- **Base-64 IDs:** 21+ chars with 2+ upper, 2+ lower, 2+ digits â†’ `"REDACTED"`
...
- Worker returns 500 â†’ `outcome='ok'` if script completed successfully
- Uncaught exception â†’ `outcome='exception'` regardless of HTTP status
- CPU limit exceeded â†’ `outcome='exceededCpu'`

```typescript
// âœ… Check outcome for script execution status
if (event.outcome === 'exception') {
  // Script threw uncaught exception
}

// âœ… Check HTTP status separately
...
```typescript
// âŒ May fail with circular references or BigInt
