...
### âŒ THE #1 GOTCHA: Caching env in Global Scope

```typescript
// âŒ DANGEROUS - env cached at deploy time
...
**âœ… Always access env per-request:**
```typescript
export default {
  async fetch(request: Request, env: Env) {
...
```typescript
// âŒ Wrong - KV returns string | null
...
**âŒ Secrets in logs:** `console.log('Key:', env.API_KEY)` - visible in dashboard
**âœ…** `console.log('Key:', env.API_KEY ? '***' : 'missing')`

**âŒ Exposing env:** `return Response.json(env)` - exposes all bindings
**âœ…** Never return env object in responses
