...
**HTTP vs Service:**
```typescript
// âŒ HTTP (slow, paid, cross-region latency)
await fetch('https://auth-worker.example.com/validate');

// âœ… Service binding (fast, free, same isolate)
...
```typescript
// âœ… Parallelize independent calls
...
**âŒ Hardcoding credentials:** `const apiKey = 'sk_live_abc123'`
**âœ…** `npx wrangler secret put API_KEY`

**âŒ Using REST API:** `fetch('https://api.cloudflare.com/.../kv/...')`
**âœ…** `env.MY_KV.get('key')`

**âŒ Polling storage:** `setInterval(() => env.KV.get('config'), 1000)`
**âœ…** Use Durable Objects for real-time state

**âŒ Large data in vars:** `{ "vars": { "HUGE_CONFIG": "..." } }` (5KB max)
**âœ…** `env.MY_KV.put('config', data)`

**âŒ Caching env globally:** `const apiKey = env.API_KEY` outside fetch()
**âœ…** Access `env.API_KEY` per-request inside fetch()
