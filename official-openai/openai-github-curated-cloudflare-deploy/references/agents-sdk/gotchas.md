...
```ts
// âŒ this.state.count++
// âœ… this.setState({...this.state, count: this.state.count + 1})
```
...
```ts
// âŒ this.sql`...WHERE id = '${userId}'`
// âœ… this.sql`...WHERE id = ${userId}`
```
...
```ts
// âŒ Returns class instance
@callable()
async getData() { return new Date(); }

// âœ… Returns serializable object
@callable()
