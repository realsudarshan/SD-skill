...
|-------|----------|---------|
| Credentials not working | Check TTL â‰¤ 48hrs | [See Troubleshooting](#issue-turn-credentials-not-working) |
...
```typescript
// âŒ BAD: IPs can change with 14-day notice
...
```typescript
// âŒ BAD: Blocked by Chrome/Firefox
urls: ['turn:turn.cloudflare.com:53']

// âœ… GOOD: Filter port 53
...
```typescript
// âŒ BAD: Credentials expire but call continues â†’ connection drops
const creds = await fetchCreds();
const pc = new RTCPeerConnection({ iceServers: creds });

// âœ… GOOD: Refresh before expiry
...
```typescript
// âŒ BAD: No recovery from TURN maintenance
pc.addEventListener('iceconnectionstatechange', () => {
  console.log('State changed:', pc.iceConnectionState);
});

// âœ… GOOD: Implement ICE restart
...
- [ ] TTL â‰¤ expected session duration (and â‰¤ 48 hours)
