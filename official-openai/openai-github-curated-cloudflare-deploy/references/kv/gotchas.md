...
**Solution:** Don't read immediately after write; return confirmation without reading or use the local value you just wrote. Writes visible immediately in same location, â‰¤60s globally

```typescript
// âŒ BAD: Read immediately after write
await env.KV.put("key", "value");
const value = await env.KV.get("key"); // May be null in other regions!

// âœ… GOOD: Use the value you just wrote
...
```typescript
// Check â†’ create pattern has race condition
...
**Free tier:**
- 100K reads/day = 3M/month âœ…
- 1K writes/day = 30K/month âœ…
- 1GB storage âœ…
...
| Propagation time | â‰¤60s | Global propagation time |
