...
**Port resolution:** explicit ports â†’ requiredPorts â†’ defaultPort â†’ port 33
...
```typescript
// âœ… Supports WebSocket upgrades
...
```typescript
// âŒ No WebSocket support
const response = await container.containerFetch(request);
```

**âš ï¸ Critical:** Use `fetch()` for WebSocket, not `containerFetch()`.
...
**âš ï¸ Don't override `alarm()` directly when using `schedule()` helper.**
...
**âš ï¸ Use `getState()` for external checks, `ctx.container.running` for internal.**
