...
- `getSandbox(namespace, id, options?)` â†’ Get/create sandbox
- `sandbox.exec(command, options?)` â†’ Execute command
- `sandbox.readFile(path)` / `writeFile(path, content)` â†’ File ops
- `sandbox.startProcess(command, options)` â†’ Background process
- `sandbox.exposePort(port, options)` â†’ Get preview URL
- `sandbox.createSession(options)` â†’ Isolated session
- `sandbox.wsConnect(request, port)` â†’ WebSocket proxy
- `sandbox.destroy()` â†’ Terminate container
- `sandbox.mountBucket(bucket, path, options)` â†’ Mount S3 storage
