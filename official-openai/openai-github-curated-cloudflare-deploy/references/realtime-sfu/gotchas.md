...
pc.oniceconnectionstatechange = async () => {
  if (pc.iceConnectionState === 'failed') {
    ...
    // Send to backend â†’ Cloudflare API
...
**Cause:** Mobile switching WiFiâ†”cellular, laptop changing networks
**Solution:**
```typescript
...
- âœ… **Never expose** `CALLS_APP_SECRET` to client
- âœ… **Validate user identity** in backend before creating sessions
- âœ… **Implement auth tokens** for session access (JWT in custom header)
- âœ… **Rate limit** session creation endpoints
- âœ… **Expire sessions** server-side after inactivity
- âœ… **Validate track IDs** before subscribing (prevent unauthorized access)
- âœ… **Use HTTPS** for all signaling (API calls)
- âœ… **Enable DTLS-SRTP** (automatic with Cloudflare, encrypts media)
- âš ï¸ **Consider E2EE** for sensitive content (implement client-side with Insertable Streams API)
