...
â†’ {sessionId, sessionDescription}
```
...
â†’ {sessionDescription, tracks: [{trackName}]}
```
...
â†’ {sessionDescription} (server offer)
```
...
â†’ {requiresImmediateRenegotiation: boolean}
```

### Get Session
```http
GET /v1/apps/{appId}/sessions/{sessionId}
â†’ {sessionId, tracks: TrackMetadata[]}
```
...
// 4. Send to backend â†’ Cloudflare API
