...
```
Data Sources â†’ Streams â†’ Pipelines (SQL) â†’ Sinks â†’ R2
                 â†‘          â†“                â†“
...
Need SQL queries on data?
  â†’ R2 Data Catalog (Iceberg)
    âœ… ACID transactions, time-travel, schema evolution
    âŒ More setup complexity (namespace, table, catalog token)

Just file storage/archival?
  â†’ R2 Storage (Parquet)
    âœ… Simple, direct file access
    âŒ No built-in SQL queries

Using external tools (Spark/Athena)?
  â†’ R2 Storage (Parquet with partitioning)
    âœ… Standard format, partition pruning for performance
    âŒ Must manage schema compatibility yourself
```
...
**Task-based routing:**
- Setup pipeline â†’ [configuration.md](./configuration.md)
- Send/query data â†’ [api.md](./api.md)
- Implement pattern â†’ [patterns.md](./patterns.md)
- Debug issue â†’ [gotchas.md](./gotchas.md)
