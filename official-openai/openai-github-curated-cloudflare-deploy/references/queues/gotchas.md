...
```typescript
// âŒ BAD: Throws error, retries entire batch
...
// âœ… GOOD: Catch per message, handle individually
async queue(batch: MessageBatch): Promise<void> {
  for (const msg of batch.messages) {
    try {
...
```typescript
// âŒ BAD: Skipped messages auto-retry forever
async queue(batch: MessageBatch): Promise<void> {
  for (const msg of batch.messages) {
...
// âœ… GOOD: Explicitly handle all messages
async queue(batch: MessageBatch): Promise<void> {
  for (const msg of batch.messages) {
    if (shouldProcess(msg.body)) {
      await process(msg.body);
...
**Decision tree:**
1. Need to view in dashboard or use pull consumer? â†’ Use `json`
2. Need Date, Map, Set, or other V8 types? â†’ Use `v8` (push consumers only)
3. Just strings? â†’ Use `text`
4. Binary data? â†’ Use `bytes`
