...
```
/functions/index.ts              â†’ example.com/
/functions/api/users.ts          â†’ example.com/api/users
/functions/api/users/[id].ts     â†’ example.com/api/users/:id
/functions/api/users/[[path]].ts â†’ example.com/api/users/* (catchall)
/functions/_middleware.ts        â†’ Runs before all routes
```
...
export const onRequestGet: PagesFunction = async ({ params }) => {
  // /files/docs/api/v1.md â†’ params.path = ["docs", "api", "v1.md"]
...
**Scope**: `functions/_middleware.ts` â†’ all; `functions/api/_middleware.ts` â†’ `/api/*` only
