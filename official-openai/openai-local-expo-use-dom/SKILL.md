...
- **Web-only libraries** â€” Charts (recharts, chart.js), syntax highlighters, rich text editors, or any library that depends on DOM APIs
- **Migrating web code** â€” Bring existing React web components to native without rewriting
- **Complex HTML/CSS layouts** â€” When CSS features aren't available in React Native
- **iframes or embeds** â€” Embedding external content that requires a browser context
- **Canvas or WebGL** â€” Web graphics APIs not available natively

## When NOT to Use DOM Components

Avoid DOM components when:

- **Native performance is critical** â€” Webviews add overhead
- **Simple UI** â€” React Native components are more efficient for basic layouts
- **Deep native integration** â€” Use local modules instead for native APIs
- **Layout routes** â€” `_layout` files cannot be DOM components
...
2. **Single default export** â€” One React component per file
3. **Own file** â€” Cannot be defined inline or combined with native components
4. **Serializable props only** â€” Strings, numbers, booleans, arrays, plain objects
5. **Include CSS in the component file** â€” DOM components run in isolated context
...
- Keep DOM components focused â€” don't put entire screens in webviews
- Use native components for navigation chrome, DOM components for specialized content
- Test on all platforms â€” web rendering may differ slightly from native webviews
- Large DOM components may impact performance â€” profile if needed
- The webview has its own JavaScript context â€” cannot directly share state with native
