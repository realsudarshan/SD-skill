...
description: Vercel Functions expert guidance â€” Serverless Functions, Edge Functions, Fluid Compute, streaming, Cron Jobs, and runtime configuration. Use when configuring, debugging, or optimizing server-side code running on Vercel.
metadata:
  priority: 8
  docs:
...
You are an expert in Vercel Functions â€” the compute layer of the Vercel platform.
...
- Cold starts: 800msâ€“2.5s (with DB connections)
...
Key benefits:
- **Optimized concurrency**: Multiple invocations on a single instance â€” up to 85% cost reduction for high-concurrency workloads
...
2. **Edge limitations**: No `fs`, no native modules, limited `crypto` â€” use Node.js runtime if needed
...
```
504 Gateway Timeout?
â”œâ”€ All plans default to 300s with Fluid Compute
â”œâ”€ Pro/Enterprise: configurable up to 800s
â”œâ”€ Long-running task?
â”‚  â”œâ”€ Under 5 min â†’ Use Fluid Compute with streaming
â”‚  â”œâ”€ Up to 15 min â†’ Use Vercel Functions with `maxDuration` in vercel.json
â”‚  â””â”€ Hours/days â†’ Use Workflow DevKit (DurableAgent or workflow steps)
â””â”€ DB query slow? â†’ Add connection pooling, check cold start, use Edge Config
```
...
â”œâ”€ Check Vercel Runtime Logs (Dashboard â†’ Deployments â†’ Functions tab)
â”œâ”€ Missing env vars? â†’ Compare `.env.local` against Vercel dashboard settings
â”œâ”€ Import error? â†’ Verify package is in `dependencies`, not `devDependencies`
â””â”€ Uncaught exception? â†’ Wrap handler in try/catch, use `after()` for error reporting
```
...
â”œâ”€ Memory exceeded? â†’ Increase `memory` in vercel.json (up to 3008 MB on Pro)
â”œâ”€ Crashed during init? â†’ Check top-level await or heavy imports at module scope
â””â”€ Edge Function crash? â†’ Check for Node.js APIs not available in Edge runtime
```
...
â”œâ”€ Using Node.js runtime? â†’ Consider Edge Functions for latency-sensitive routes
â”œâ”€ Large function bundle? â†’ Audit imports, use dynamic imports, tree-shake
â”œâ”€ DB connection in cold start? â†’ Use connection pooling (Neon serverless driver)
â””â”€ Enable Fluid Compute to reuse warm instances across requests
```
...
â”œâ”€ Edge Functions have 25s hard limit (not configurable)
â”œâ”€ Move heavy computation to Node.js Serverless Functions
â””â”€ Use streaming to start response early, process in background with `waitUntil`
```
