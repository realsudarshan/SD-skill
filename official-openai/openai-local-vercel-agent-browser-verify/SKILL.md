...
description: Automated browser verification for dev servers. Triggers when a dev server starts to run a visual gut-check with agent-browser â€” verifies the page loads, checks for console errors, validates key UI elements, and reports pass/fail before continuing.
metadata:
  priority: 2
  docs:
...
1. **Page loads** â€” `agent-browser open` succeeds without timeout
2. **No blank page** â€” snapshot shows meaningful content (not empty body)
3. **No error overlay** â€” no Next.js/Vite error overlay detected
4. **Console errors** â€” evaluate `document.querySelectorAll('[data-nextjs-dialog]')` for error modals
5. **Key elements render** â€” snapshot `-i` shows expected interactive elements
6. **Navigation works** â€” if multiple routes exist, verify at least the home route
...
|--------------|-------------|-------------|
| Spinner / loading forever | No recent function invocations | API route not being called â€” check fetch URL in client code |
| Spinner / loading forever | Function started but no step logs | Workflow step is stuck â€” add `console.log` at step entry/exit |
| Blank page, no errors | Build succeeded, no runtime errors | Hydration issue or missing data â€” check SSR vs client rendering |
| Network request pending | 504 Gateway Timeout in logs | Function timeout â€” increase `maxDuration` or optimize step |
| Console: "Failed to fetch" | OIDC/credential error in logs | Missing `vercel env pull` â€” run `vercel link && vercel env pull` |
| Error overlay visible | Stack trace in runtime logs | Read the server error â€” it usually has more detail than the client |
...
Report: "Dev server verified â€” page loads, no errors detected, key UI elements render correctly."
