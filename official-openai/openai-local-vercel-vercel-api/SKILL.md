...
# Vercel API â€” Connected App & REST API
...
Auth:      OAuth 2.1 (automatic â€” agent is prompted to authorize on first use)
```
...
```
1. List recent deployments â†’ find the failed one
2. Inspect deployment â†’ get error summary
3. Query build logs â†’ identify root cause
...
```
1. Get project details â†’ check framework, build settings, root directory
2. List environment variables â†’ verify required vars are set per environment
3. List domains â†’ confirm production domain is correctly assigned
4. Check deployment logs â†’ look for runtime warnings
```

#### Search documentation

```
1. Search Vercel docs for a topic â†’ get relevant pages
2. Read specific doc page â†’ extract configuration examples
...
```
1. Query function logs â†’ find slow invocations
2. Inspect deployment â†’ check function region, runtime, memory
...
MCP servers deployed on Vercel use **Streamable HTTP** transport (replaced SSE in March 2025 MCP spec) â€” cuts CPU usage vs SSE with no persistent connections required. Used in production by Zapier, Composio, Vapi, and Solana.
...
for (const d of deployments) {
  console.log(`${d.url} â€” ${d.state} â€” ${d.created}`);
}
```
...
for (const d of domains) {
  console.log(`${d.name} â€” verified: ${d.verified}`);
}
```
...
Drains forward logs, traces, speed insights, and web analytics data to external endpoints. All drain management is REST API or Dashboard (`https://vercel.com/dashboard/{team}/~/settings/log-drains`) only â€” no CLI commands exist.
...
> For payload schemas (JSON, NDJSON), signature verification, and vendor integration setup, see `â¤³ skill: observability`.

### Runtime Logs (`/v3/deployments/:id/events`)

Stream runtime logs for a deployment. The response uses `application/stream+json` â€” each line is a separate JSON object. Always set a timeout to avoid hanging on long-lived streams.
...
// Parse as NDJSON â€” see observability skill for streaming code patterns
```
...
This bridges the gap between the read-only MCP server and the full REST API â€” agents can perform write operations without needing `@vercel/sdk` or manual `curl` with tokens.
...
- **CLI operations** â†’ `â¤³ skill: vercel-cli`
- **Function configuration** â†’ `â¤³ skill: vercel-functions`
- **Storage APIs** â†’ `â¤³ skill: vercel-storage`
- **Firewall rules** â†’ `â¤³ skill: vercel-firewall`
- **AI SDK MCP client** â†’ `â¤³ skill: ai-sdk` (section: MCP Integration)
- **Drains, log streaming, analytics export** â†’ `â¤³ skill: observability`
