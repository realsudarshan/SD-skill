---
name: cms
description: Headless CMS integration guidance â€” Sanity (native Vercel Marketplace), Contentful, DatoCMS, Storyblok, and Builder.io. Covers studio setup, content modeling, preview mode, revalidation webhooks, and Visual Editing. Use when building content-driven sites with a headless CMS on Vercel.
metadata:
  priority: 4
  docs:
...
You are an expert in integrating headless CMS platforms with Vercel-deployed applications â€” covering Sanity (native Vercel Marketplace), Contentful, DatoCMS, Storyblok, and Builder.io.
...
- `SANITY_PROJECT_ID` â€” Sanity project identifier
- `SANITY_DATASET` â€” dataset name (usually `production`)
- `SANITY_API_TOKEN` â€” read/write API token
- `NEXT_PUBLIC_SANITY_PROJECT_ID` â€” client-side project ID
- `NEXT_PUBLIC_SANITY_DATASET` â€” client-side dataset name
...
export const { sanityFetch, SanityLive } = defineLive({
  client,
  // Required for draft content in Visual Editing â€” use a Viewer role token
...
Configure the webhook in Sanity at **Settings â†’ API â†’ Webhooks** pointing to `https://your-site.vercel.app/api/revalidate`.
...
- **On-demand revalidation and caching** â†’ `â¤³ skill: runtime-cache`
- **Draft mode and middleware patterns** â†’ `â¤³ skill: routing-middleware`
- **Environment variable management** â†’ `â¤³ skill: env-vars`
- **Image optimization** â†’ `â¤³ skill: nextjs`
...
- [next-sanity v11â†’v12 Migration](https://github.com/sanity-io/next-sanity/releases)
