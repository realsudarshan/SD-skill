...
description: "Vercel Services â€” deploy multiple services within a single Vercel project. Use for monorepo layouts or when combining a backend (Python, Go) with a frontend (Next.js, Vite) in one deployment."
metadata:
  priority: 7
  docs:
...
| `memory`       | No       | Max available RAM in MB (128â€“10,240).                      |
| `maxDuration`  | No       | Execution timeout in seconds (1â€“900).                      |
...
Do not add unknown fields â€” they will cause the build to fail.
...
Set `entrypoint` to the service **directory** (e.g. `"backend"`), not a file. **Must** set `"framework": "go"` explicitly in `vercel.json` â€” auto-detection does not work for Go services. Dependencies in `go.mod` in the service directory.
...
2. Adapt the structure to the user's chosen runtimes â€” services can use any supported runtime, not just the ones in the reference.
3. Define service routes **without** the route prefix â€” Vercel strips the prefix before forwarding.
...
1. Go to Project Settings â†’ Build & Deployment â†’ Framework Preset
2. Select **Services** from the dropdown
3. Redeploy

### Routes return unexpected results

1. Ensure all services are picked up by `vercel dev` â€” check logs. If a service is missing, verify `vercel.json`. Try setting `framework` explicitly.
