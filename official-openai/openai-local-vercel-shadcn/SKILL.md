...
You are an expert in shadcn/ui â€” a collection of beautifully designed, accessible, and customizable React components built on Radix UI primitives and Tailwind CSS. Components are added directly to your codebase as source code, not installed as a dependency.
...
Options:
- `-d, --defaults` â€” **Use default configuration, skip all interactive prompts** (REQUIRED for CI/agent use)
- `-y, --yes` â€” Skip confirmation prompts (does NOT skip library selection â€” use `-d` instead)
- `-f, --force` â€” Force overwrite existing configuration
- `-t, --template` â€” Scaffold full project template (`next`, `vite`, `react-router`, `astro`, `laravel`, `tanstack-start`)
- `--preset` â€” Apply a design system preset (colors, theme, icons, fonts, radius) as a single shareable code
- `--base` â€” Choose primitive library: `radix` (default) or `base-ui`
- `--monorepo` â€” Set up a monorepo structure

> **WARNING**: `-y`/`--yes` alone does NOT make init fully non-interactive â€” it still prompts for component library selection. Always use `-d` to skip ALL prompts.
...
Options:
- `-o, --overwrite` â€” Overwrite existing files
- `-p, --path` â€” Custom install path
- `-a, --all` â€” Install all components
- `--dry-run` â€” Preview what will be added without writing files
- `--diff` â€” Show diff of changes when updating existing components
- `--view` â€” Display a registry item's source code inline
...
# Show project diagnostics â€” config, installed components, dependencies
...
> **`shadcn docs`** gives coding agents the context to use primitives correctly â€” returns code examples, API reference, and usage patterns inline.
...
```tsx
// OLD â€” individual packages
import * as DialogPrimitive from "@radix-ui/react-dialog"

// NEW â€” unified package
...
shadcn/ui now supports **Base UI** as an alternative to Radix UI for the underlying primitive library. Components look and behave the same way regardless of which library you choose â€” only the underlying implementation changes.
...
| `registry:base` | Full design system payload â€” components, deps, CSS vars, fonts, config |
...
`shadcn init` rewrites `globals.css` and may introduce `--font-sans: var(--font-sans)` â€” a circular self-reference that breaks font loading. Tailwind v4's `@theme inline` resolves CSS custom properties at **parse time**, not runtime â€” so even `var(--font-geist-sans)` won't work because Next.js injects that variable via className at runtime.

**The fix**: Use literal font family names in `@theme inline`:

```css
/* In @theme inline â€” CORRECT (literal names) */
--font-sans: "Geist", "Geist Fallback", ui-sans-serif, system-ui, sans-serif;
--font-mono: "Geist Mono", "Geist Mono Fallback", ui-monospace, monospace;

/* WRONG â€” circular, resolves to nothing */
--font-sans: var(--font-sans);

/* ALSO WRONG â€” @theme inline can't resolve runtime CSS variables */
...
```tsx
// layout.tsx â€” font variables on <html>, not <body>
...
```tsx
// WRONG â€” no size variant exists
<Avatar size="lg" />  // âŒ TypeScript error / silently ignored

// CORRECT â€” use Tailwind
...
This applies to most shadcn components â€” they use Tailwind classes for sizing, not variant props. If you need reusable size variants, add them yourself via `cva` in the component source.
...
```tsx
// components/ui/button.tsx â€” add your custom variant
...
- **Next.js** â€” Full support (App Router + Pages Router)
- **Vite** â€” Full support
- **React Router** â€” Full support
- **Astro** â€” Full support
- **Laravel** â€” Full support (via Inertia)
- **TanStack Start** â€” Full support
...
Build custom presets on `shadcn/create` â€” preview how colors, fonts, and radius apply to real components before publishing.
