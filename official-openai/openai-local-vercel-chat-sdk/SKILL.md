---
name: chat-sdk
description: Vercel Chat SDK expert guidance. Use when building multi-platform chat bots â€” Slack, Telegram, Microsoft Teams, Discord, Google Chat, GitHub, Linear â€” with a single codebase. Covers the Chat class, adapters, threads, messages, cards, modals, streaming, state management, and webhook setup.
metadata:
  priority: 8
  docs:
...
- `Table` / `TableRow` / `TableCell` â€” native per-platform table rendering (**new â€” Mar 6, 2026**; see below)
- `Text` styles (`default`, `muted`, `success`, `warning`, `danger`, `code`)

### Table â€” Per-Platform Rendering (New â€” Mar 6, 2026)
...
7. Fallback adapters now convert markdown to each platform's native format at every intermediate edit â€” users no longer see raw `**bold**` syntax during streaming.
