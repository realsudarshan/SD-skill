...
2. Run `pnpm migrate` (not `db:push`) â€” it runs `prisma format` + `prisma generate` + `prisma db push`.
...
4. Optional services (Stripe, Resend, PostHog, etc.) can be skipped initially â€” but remove their `@repo/*` imports from app `env.ts` files to avoid validation errors.
5. Deploy as 3 separate Vercel projects with root directories `apps/app`, `apps/api`, `apps/web`.

=> skill: next-forge â€” Full next-forge monorepo guide
