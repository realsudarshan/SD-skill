...
**Kotlin difference** â€” uses `bundleOf`:
...
**Kotlin difference** â€” uses `class` instead of `struct`, optional fields need explicit `= null`:
...
**Kotlin difference** â€” uses `enum class` with explicit `value` property:
...
**Kotlin difference** â€” takes `RuntimeContext` in constructor, override `sharedObjectDidRelease()` for cleanup:
