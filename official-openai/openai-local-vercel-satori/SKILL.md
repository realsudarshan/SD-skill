---
name: satori
description: Expert guidance for Satori â€” Vercel's library that converts HTML and CSS to SVG, commonly used to generate dynamic OG images for Next.js and other frameworks.
metadata:
  priority: 4
  docs:
...
# Satori â€” HTML/CSS to SVG for OG Images
...
```bash
# For Next.js projects (recommended â€” includes Satori + PNG rendering)
...
## Next.js App Router â€” OG Image Route (Recommended)
...
**Supported:**
- `display: flex` (default â€” all elements are flex containers)
...
**Not supported:**
- `display: grid` â€” use nested flex containers instead
...
Fonts must be loaded explicitly â€” there are no default system fonts:
...
1. **Use `next/og` in Next.js projects** â€” it re-exports `ImageResponse` with built-in optimizations
2. **Always set `runtime = 'edge'`** â€” Satori and `@vercel/og` are designed for edge runtimes
3. **Use `display: 'flex'` everywhere** â€” Satori defaults to flex layout, no block or grid support
4. **Load fonts explicitly** â€” no system fonts are available; bundle `.ttf`/`.woff` files or fetch from CDN
5. **Standard OG dimensions are 1200Ã—630** â€” this is the most widely supported size
6. **Use convention files for automatic `<meta>` tags** â€” `opengraph-image.tsx` and `twitter-image.tsx`
7. **Inline styles only** â€” Satori does not support external CSS or CSS-in-JS libraries
...
- [Next.js Metadata â€” OG Images](https://nextjs.org/docs/app/api-reference/file-conventions/metadata/opengraph-image)
