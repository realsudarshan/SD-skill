---
name: json-render
description: AI chat response rendering guidance â€” handling UIMessage parts, tool call displays, streaming states, and structured data presentation. Use when building custom chat UIs, rendering tool results, or troubleshooting AI response display issues.
metadata:
  priority: 4
  docs:
...
You are an expert in rendering AI SDK v6 chat responses â€” UIMessage parts, tool call results, streaming states, and structured data display in React applications.
...
â¤³ skill: ai-elements â€” Full component library for AI interfaces
      ...
      {messages.map((message) => (
        <div key={message.id}>
          {message.parts?.map((part, i) => {
            // 1. Text parts â€” render as formatted text
            ...
            // 2. Tool parts â€” type is "tool-<toolName>"
...
```tsx
// WRONG â€” shows raw JSON
<div>{message.content}</div>

// RIGHT â€” renders each part type
{message.parts?.map((part, i) => {
...
**Fix**: Use `await convertToModelMessages(messages)` â€” it's async in v6.
...
Building a chat UI with AI SDK v6?
  â””â”€ Want pre-built components?
       â””â”€ Yes â†’ Use AI Elements (â¤³ skill: ai-elements)
       â””â”€ No â†’ Manual rendering with parts iteration
            â””â”€ Tool results look like JSON?
                 â””â”€ Create ToolResultCard components for each tool's output shape
            â””â”€ Text not rendering?
                 â””â”€ Check part.type === 'text' and use part.text
            â””â”€ Server errors?
                 â””â”€ Check: await convertToModelMessages(), toUIMessageStreamResponse()
```
