---
name: runtime-cache
description: Vercel Runtime Cache API guidance â€” ephemeral per-region key-value cache with tag-based invalidation. Shared across Functions, Routing Middleware, and Builds. Use when implementing caching strategies beyond framework-level caching.
metadata:
  priority: 6
  docs:
...
You are an expert in the Vercel Runtime Cache â€” an ephemeral caching layer for serverless compute.
...
- `cache.expireTag()` â€” operates on Runtime Cache only
- `invalidateByTag()` / `dangerouslyDeleteByTag()` â€” purges CDN + Runtime + Data caches
...
- `'use cache'` (no `: remote`) â€” in-memory only, ephemeral per instance
- `'use cache: remote'` â€” stores in Vercel Runtime Cache
...
Monitor hit rates, invalidation patterns, and storage usage in the Vercel Dashboard under **Observability â†’ Runtime Cache**. The CDN dashboard (March 5, 2026) provides a unified view of global traffic distribution, cache performance metrics, a redesigned purging interface, and **project-level routing** â€” update response headers or rewrite to external APIs without triggering a new deployment. Project-level routes are available on all plans and take effect instantly.

## When to Use

- Caching API responses or computed data across functions in a region
- Tag-based invalidation when content changes (CMS webhook â†’ expire tag)
- Reducing database load for frequently accessed data
- Cross-function data sharing within a region

## When NOT to Use

- Framework-level page caching â†’ use Next.js Cache Components (`'use cache'`)
- Persistent storage â†’ use a database (Neon, Upstash)
- CDN-level full response caching â†’ use `Cache-Control` / `Vercel-CDN-Cache-Control` headers
- Cross-region shared state â†’ use a database
- User-specific data that differs per request

## References

- ðŸ“– docs: https://vercel.com/docs/runtime-cache
- ðŸ“– changelog: https://vercel.com/changelog/introducing-the-runtime-cache-api
- ðŸ“– CLI cache: https://vercel.com/docs/cli/cache
- ðŸ“– CDN cache purging: https://vercel.com/docs/cdn-cache/purge
