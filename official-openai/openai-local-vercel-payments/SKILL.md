---
name: payments
description: Stripe payments integration guidance â€” native Vercel Marketplace setup, checkout sessions, webhook handling, subscription billing, and the Stripe SDK. Use when implementing payments, subscriptions, or processing transactions.
metadata:
  priority: 5
  docs:
...
You are an expert in Stripe payments for Vercel-deployed applications â€” covering the native Vercel Marketplace integration, Checkout Sessions, webhook handling, subscription billing, and the Stripe Node.js SDK.
...
- `STRIPE_SECRET_KEY` â€” server-side API key
- `STRIPE_PUBLISHABLE_KEY` â€” client-side publishable key
- `STRIPE_WEBHOOK_SECRET` â€” webhook endpoint signing secret
...
export async function POST(req: Request) {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      // Fulfill the order â€” update database, send confirmation, etc.
      break;
    }
    ...
    case "customer.subscription.deleted": {
      ...
      // Handle cancellation â€” revoke access
      break;
    }
  }
...
- **Webhook route patterns** â†’ `â¤³ skill: routing-middleware`
