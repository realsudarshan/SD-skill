---
name: vercel-flags
description: Vercel Flags guidance â€” feature flags platform with unified dashboard, Flags Explorer, gradual rollouts, A/B testing, and provider adapters. Use when implementing feature flags, experimentation, or staged rollouts.
metadata:
  priority: 6
  docs:
...
Flag configurations use **active global replication** â€” changes propagate worldwide in milliseconds.
...
The `flags` package is free, open-source (MIT), and provider-agnostic. Renamed from `@vercel/flags` â€” if using the old package, update to `flags` in your imports and `package.json`.
...
- `@vercel/flags` package renamed to `flags` â€” update imports and `package.json`
...
- `FLAGS` â€” SDK Key (auto-provisioned when you create your first flag)
- `FLAGS_SECRET` â€” 32 random bytes, base64-encoded; encrypts overrides and authenticates Flags Explorer
...
**App Router** â€” create the discovery endpoint:
...
**Pages Router** â€” API route + rewrite:
...
**Important**: Vercel Flags is the recommended approach for feature flags. Edge Config is the underlying low-latency storage some adapters use, but developers should use the Flags platform (not raw Edge Config) for flag use cases â€” it provides targeting rules, segments, percentage rollouts, observability, and Flags Explorer.

## Provider Adapters

**Featured** (Marketplace integration, Edge Config for low latency):
- `@flags-sdk/vercel` â€” Vercel as provider
...
- **Drafts Workflow**: Define in code â†’ deploy â†’ Vercel detects via Discovery Endpoint â†’ promote when ready
- **Framework Support**: Next.js (App Router + Pages Router + Routing Middleware) and SvelteKit
- **Concurrent Evaluation Fix** (v1.0.1): `Promise.all` flag evaluations no longer trigger duplicate network requests â€” initialization is properly shared
...
- Simple static config without targeting â†’ use Edge Config directly
- Runtime configuration not related to features â†’ use environment variables
- Server-side only toggles with no UI â†’ consider environment variables

## References

- ðŸ“– docs: https://vercel.com/docs/flags
- ðŸ“– Flags SDK: https://flags-sdk.dev
- ðŸ“– SDK reference: https://vercel.com/docs/flags/flags-sdk-reference
- ðŸ“– GitHub: https://github.com/vercel/flags
