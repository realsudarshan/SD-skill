---
name: nextjs
description: Next.js App Router expert guidance. Use when building, debugging, or architecting Next.js applications â€” routing, Server Components, Server Actions, Cache Components, layouts, middleware/proxy, data fetching, rendering strategies, and deployment on Vercel.
metadata:
  priority: 5
  docs:
...
**If the target directory contains ANY non-Next.js files** (`.git/`, config files, etc.), you **MUST** add `--force`. Without it, `create-next-app` will fail with "The directory contains files that could conflict" and block scaffolding. **Check the directory first** â€” if it has anything in it, use `--force`:
...
`shadcn init` rewrites `globals.css` with `--font-sans: var(--font-sans)` in `@theme inline` â€” a circular reference that falls back to Times/serif. Even `var(--font-geist-sans)` doesn't work because Tailwind v4's `@theme inline` resolves at **CSS parse time**, not runtime.
...
```css
@theme inline {
  /* CORRECT â€” literal names that @theme can resolve at parse time */
...
```tsx
// app/layout.tsx â€” CORRECT
<html lang="en" className={`${geistSans.variable} ${geistMono.variable}`}>
  <body className="antialiased">
```

```tsx
// app/layout.tsx â€” WRONG (default scaffold output)
<html lang="en">
...
### File Conventions
- `layout.tsx` â€” Persistent wrapper, preserves state across navigations
- `page.tsx` â€” Unique UI for a route, makes route publicly accessible
- `loading.tsx` â€” Suspense fallback shown while segment loads
- `error.tsx` â€” Error boundary for a segment
- `not-found.tsx` â€” 404 UI for a segment
- `route.ts` â€” API endpoint (Route Handler)
- `template.tsx` â€” Like layout but re-mounts on navigation
- `default.tsx` â€” Fallback for parallel routes

### Routing
- Dynamic segments: `[id]`, catch-all: `[...slug]`, optional catch-all: `[[...slug]]`
- Route groups: `(group)` â€” organize without affecting URL
- Parallel routes: `@slot` â€” render multiple pages in same layout
- Intercepting routes: `(.)`, `(..)`, `(...)`, `(..)(..)` â€” modal patterns
...
```tsx
// Default â€” cached in the deployment's local data cache
'use cache'

// Remote cache â€” shared across all deployments and regions (Vercel only)
'use cache: remote'

// Private cache â€” per-request cache, never shared between users
'use cache: private'
```
...
In Next.js 16, `middleware.ts` is renamed to `proxy.ts`. It runs **exclusively on the Node.js runtime** â€” the Edge runtime is not supported in proxy and cannot be configured.
...
**Constraints**:
- Proxy can only **rewrite**, **redirect**, or **modify headers** â€” it cannot return full response bodies. Use Route Handlers for that.
- Config flags are renamed: `skipMiddlewareUrlNormalize` â†’ `skipProxyUrlNormalize`.
...
- **OpenNext note**: OpenNext doesn't support `proxy.ts` yet â€” keep using `middleware.ts` if self-hosting with OpenNext.
...
1. **Turbopack File System Caching (Stable)** â€” Compiler artifacts are now cached on disk between `next dev` restarts, delivering up to 14Ã— faster startup on large projects. Enabled by default, no config needed. File system caching for `next build` is planned next.
2. **Bundle Analyzer (Experimental)** â€” New built-in bundle analyzer works with Turbopack. Offers route-specific filtering, import tracing, and RSC boundary analysis to identify bloated dependencies in both server and client bundles. Enable with `experimental.bundleAnalyzer: true` in `next.config`.
3. **`next dev --inspect`** â€” Debug your dev server without global `NODE_OPTIONS=--inspect`. Applies the inspector only to the relevant process.
4. **`next upgrade` command** â€” New CLI command to simplify version upgrades: `npx @next/codemod@canary upgrade latest`.
5. **Transitive External Dependencies** â€” Turbopack correctly resolves and externalizes transitive deps in `serverExternalPackages` without extra config.
6. **20 MB smaller installs** â€” Streamlined Turbopack caching layer reduces `node_modules` footprint.
...
function ChatRoom({ roomId, theme }: { roomId: string; theme: string }) {
  ...
  }, [roomId]) // theme is NOT a dependency â€” onConnected reads it via useEffectEvent
}
```
...
Use for: tabbed interfaces, modals, sidebars, background tasks â€” anywhere you need to maintain component state without keeping everything actively rendered. When `mode="hidden"`, effects are suspended (not running in the background).

### View Transitions API

React 19.2 supports the browser View Transitions API for animating elements across navigations. Next.js 16 has built-in support â€” elements can animate between route changes without manual transition logic.
...
**Impact**: A page with 50 product links that share a layout downloads ~198KB instead of ~2.4MB â€” a 92% reduction in prefetch network transfer.
...
- Full import chain tracing â€” see exactly why a module is included
- Traces imports across RSC boundaries and dynamic imports
- No application build required â€” analyzes module graph directly
...
1. **Turbopack File System Caching for `next build`** â€” Extending the stable `next dev` FS cache to production builds for faster CI.
2. **Proxy refinements** â€” Continued iteration on `proxy.ts` (the Node.js-runtime replacement for `middleware.ts` introduced in 16.0). The proxy API is stabilizing with improved request context and streaming support.
3. **React Compiler optimizations** â€” Further automatic memoization improvements building on the stable React Compiler in 16.0.
...
1. **Async Request APIs**: `cookies()`, `headers()`, `params`, `searchParams` are all async â€” must `await` them
2. **Proxy replaces Middleware**: Rename `middleware.ts` â†’ `proxy.ts`, runs on Node.js only (Edge not supported)
...
```tsx
// WRONG â€” React 19 strict mode complains
const ref = useRef()       // âŒ missing initial value
const ref = useRef<HTMLDivElement>()  // âŒ still missing

// CORRECT
const ref = useRef<HTMLDivElement>(null)  // âœ…
const ref = useRef(0)                     // âœ…
```
...
### CVE-2025-66478 / CVE-2025-55182 â€” Remote Code Execution (CVSS 10.0, Critical)

A deserialization vulnerability in the React Server Components (RSC) "Flight" protocol allows unauthenticated remote code execution via crafted HTTP requests. Near-100% exploit reliability against default configurations. **Actively exploited in the wild.** No workaround â€” upgrade is required. Rotate all application secrets after patching.
...
### CVE-2025-55184 â€” Denial of Service (CVSS 7.5, High)
...
### CVE-2025-55183 â€” Source Code Exposure (CVSS 5.3, Medium)
...
### CVE-2026-23864 â€” Denial of Service via Memory Exhaustion (CVSS 7.5, High)

Disclosed January 26, 2026. The original DoS fix for CVE-2025-55184 was incomplete â€” additional vectors allow specially crafted HTTP requests to Server Function endpoints to crash the server via out-of-memory exceptions or excessive CPU usage. No authentication required.

### CVE-2025-29927 â€” Middleware Authorization Bypass (CVSS 9.1, Critical)
...
â”œâ”€ Content changes less than once per day?
â”‚  â”œâ”€ Same for all users? â†’ SSG (`generateStaticParams`)
â”‚  â””â”€ Personalized? â†’ SSG shell + client fetch for personalized parts
â”‚
â”œâ”€ Content changes frequently but can be slightly stale?
â”‚  â”œâ”€ Revalidate on schedule? â†’ ISR with `revalidate: N` seconds
â”‚  â””â”€ Revalidate on demand? â†’ `revalidateTag()` or `revalidatePath()`
â”‚
â”œâ”€ Content must be fresh on every request?
â”‚  â”œâ”€ Cacheable per-request? â†’ Cache Components (`'use cache'` + `cacheLife`)
â”‚  â”œâ”€ Personalized per-user? â†’ SSR with Streaming (Suspense boundaries)
â”‚  â””â”€ Real-time? â†’ Client-side with SWR/React Query + SSR for initial load
â”‚
â””â”€ Mostly static with one dynamic section?
   â””â”€ Partial Prerendering: static shell + Suspense for dynamic island
```
...
- **`ImageResponse`** â€” Import from `next/og` (re-exports `@vercel/og`). Renders JSX to PNG/SVG images.
- **Edge runtime** â€” OG image routes run on the Edge runtime by default. Export `runtime = 'edge'` explicitly for clarity.
- **Exports** â€” `alt`, `size`, and `contentType` configure the generated `<meta>` tags automatically.
- **Static or dynamic** â€” Without params, the image is generated at build time. With dynamic segments, it generates per-request.
- **Supported CSS** â€” Satori supports a Flexbox subset. Use inline `style` objects (no Tailwind). `display: 'flex'` is required on containers.
- **Fonts** â€” Load custom fonts via `fetch` and pass to `ImageResponse` options: `{ fonts: [{ name, data, style, weight }] }`.
- **Twitter fallback** â€” If no `twitter-image.tsx` exists, `opengraph-image.tsx` is used for Twitter cards too.
...
- Server Components â†’ Serverless/Edge Functions automatically
