...
Pass a `"provider/model"` string to the `model` parameter â€” the AI SDK automatically routes it through the AI Gateway:
...
No `gateway()` wrapper or additional package needed. The `gateway()` function is an optional explicit wrapper â€” only needed when you use `providerOptions.gateway` for routing, failover, or tags:
...
# Enable AI Gateway in Vercel dashboard: https://vercel.com/{team}/{project}/settings â†’ AI Gateway
vercel env pull .env.local     # Provisions VERCEL_OIDC_TOKEN automatically
```

### How It Works

1. `vercel env pull` writes a `VERCEL_OIDC_TOKEN` to `.env.local` â€” a short-lived JWT (~24h)
...
4. On Vercel deployments, OIDC tokens are auto-refreshed â€” zero maintenance
...
- **Static knowledge queries**: FAQs, translations, factual lookups â€” cache aggressively
- **User-specific conversations**: Do not cache â€” each response depends on conversation history
...
Configure rate limits at `https://vercel.com/{team}/{project}/settings` â†’ **AI Gateway** â†’ **Rate Limits**:
...
In the Vercel dashboard at `https://vercel.com/{team}/{project}/settings` â†’ **AI Gateway**:

1. Navigate to **AI Gateway â†’ Usage & Budgets**
...
The AI SDK's `usage` field on responses gives actual token counts after each request â€” store these for historical tracking and cost analysis.
...
if (APICallError.isInstance(error) && error.statusCode === 402) {
  // Budget exceeded â€” degrade gracefully
...
- **Vercel Dashboard** at `https://vercel.com/{team}/{project}/ai` â†’ **Logs** â€” filter by model, user, tag, status, date range
...
If your provider API key hits its quota, the gateway tries the next provider in the `order` list. Monitor this in logs â€” persistent quota errors indicate you need to increase limits with the provider.

### Invalid model identifier

```ts
// Bad â€” model doesn't exist
model: 'openai/gpt-99'  // Returns 400 with descriptive error

// Good â€” use models listed in Vercel docs
                  ...
                  Need audit logging?
                    ...
                    â””â”€ No
                        Using a single provider with provider-specific features?
                          â””â”€ Yes â†’ Use direct provider SDK
                          â””â”€ No â†’ Use Gateway (simplifies code)
```
...
- Production applications â€” failover and observability are essential
- Multi-tenant SaaS â€” per-user tracking and rate limiting
- Teams with cost accountability â€” tag-based budgeting

## Latest Model Availability

**GPT-5.4** (added March 5, 2026) â€” agentic and reasoning leaps from GPT-5.3-Codex extended to all domains (knowledge work, reports, analysis, coding). Faster and more token-efficient than GPT-5.2.
...
- **Zero markup**: Tokens at exact provider list price â€” no middleman markup, whether using Vercel-managed keys or Bring Your Own Key (BYOK)
- **Free tier**: Every Vercel team gets **$5 of free AI Gateway credits per month** (refreshes every 30 days, starts on first request). No commitment required â€” experiment with LLMs indefinitely on the free tier
...
```ts
// Text â€” through gateway
const { text } = await generateText({
  model: 'openai/gpt-5.4',
  prompt: 'Hello',
})

// Image â€” through gateway (multimodal LLMs return images in result.files)
const result = await generateText({
...
// Image-only models â€” through gateway with experimental_generateImage
...
**Default image model**: `google/gemini-3.1-flash-image-preview` â€” fast multimodal image generation via gateway.
...
|----------|-------------|
| Production app with AI features | Yes â€” failover, cost tracking |
| Prototyping with single provider | Optional â€” direct provider works fine |
| Multi-provider setup | Yes â€” unified routing |
| Need provider-specific features | Use direct provider SDK + Gateway as fallback |
| Cost tracking and budgeting | Yes â€” user tracking and tags |
| Multi-tenant SaaS | Yes â€” per-user rate limiting and audit |
| Compliance requirements | Yes â€” audit logging and log drains |
