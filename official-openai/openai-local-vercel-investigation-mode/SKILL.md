...
description: "Orchestrated debugging coordinator. Triggers on frustration signals (stuck, hung, broken, waiting) and systematically triages: runtime logs â†’ workflow status â†’ browser verify â†’ deploy/env. Reports findings at every step."
metadata:
  priority: 8
  docs:
...
# Investigation Mode â€” Orchestrated Debugging
...
1. **Tell the user what you are checking** â€” "I'm checking the runtime logs for errorsâ€¦"
2. **Share the evidence you found** â€” paste the relevant log line, status, error, or screenshot
3. **Explain the next step** â€” "The logs show a timeout on the DB call. I'll check the connection pool next."

Never silently move between steps. The user is already frustrated â€” silence makes it worse.

## Triage Order

Work through these in order. Stop as soon as you find the root cause.

### 1. Runtime Logs (check first â€” most issues leave traces here)
...
- **Browser console**: Open DevTools â†’ Console tab for client-side errors
- **If no logs exist**: This is the problem. Add logging before continuing (see "Add Logging" below)

Tell the user: "Checking runtime logsâ€¦" â†’ share what you found â†’ explain next step.
...
- Look for runs stuck in `running` state â€” likely a missing `await` or unresolved promise
- Check individual run details: `vercel workflow runs get <run-id>`
- Look for failed steps, retry exhaustion, or timeout errors

Tell the user: "Checking workflow run statusâ€¦" â†’ share the run state â†’ explain next step.
...
Tell the user: "Taking a browser screenshot to see the current stateâ€¦" â†’ share the screenshot â†’ explain what you see.

### 4. Deploy / Environment Status

- `vercel inspect <deployment-url>` â€” check build output, function regions, environment
- `vercel ls` â€” verify the latest deployment succeeded
- Check for environment variable mismatches between local and production
- Verify the correct branch/commit is deployed

Tell the user: "Checking deployment statusâ€¦" â†’ share the deployment state â†’ explain findings.

## Stop Condition

**Stop investigating when:**
- You find a high-confidence root cause (specific error, missing env var, failed step, etc.)
- Two consecutive triage steps produce no signal â€” report what you checked and that you found no evidence, then ask the user for more context
...
If the investigation reveals insufficient observability, **add structured logging immediately** â€” you cannot debug what you cannot see.

```typescript
// API routes â€” wrap handlers with try/catch + logging
...
```typescript
// Workflow steps â€” log entry/exit of every step
