...
## Document structure
When creating a new spreadsheet, compose a clear visual layout with distinct zones: a title/header area, the primary table or working region, andâ€”when space and task type allowâ€”a secondary adjacent to the main table to cover summary section or instructions for how to use the sheet. When appropriate, design the sheet so it reads like a structured document, not just a matrix of cells. Use layout, scale, and selective merging to create clear sections and give headers enough room to breathe.
