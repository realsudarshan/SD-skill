...
Geist Pixel is intended for display sizes only â€” use Geist Sans for body text and Geist Mono for code.
...
1. **Optimized for Next.js** â€” works seamlessly with `next/font` for zero-layout-shift font loading
2. **Three families** â€” Geist Sans for UI text, Geist Mono for code, Geist Pixel for decorative display
3. **CSS variables** â€” `--font-geist-sans` and `--font-geist-mono` for flexible styling
4. **Variable font** â€” single file supports all weights (100â€“900)
5. **Self-hosted** â€” fonts are bundled with your app, no external requests
6. **Import paths** â€” use `geist/font/sans` and `geist/font/mono` (not `geist/font`)
7. **Coding ligatures** â€” opt-in via Stylistic Set 11 (no longer default)
