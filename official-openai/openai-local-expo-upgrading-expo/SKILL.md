...
- ./references/react-19.md -- SDK +54: React 19 changes (useContext â†’ use, Context.Provider â†’ Context, forwardRef removal)
...
**First check if `ios/` and `android/` directories exist in the project.** If neither directory exists, the project uses Continuous Native Generation (CNG) and native projects are regenerated at build time â€” skip this section and "Clear caches for bare workflow" entirely.
...
- Enable React Compiler in SDK 54+ by adding `"experiments": { "reactCompiler": true }` to app.json â€” it's stable and recommended
