...
- **Immediate loading**: Audio loads immediately when the hook mountsâ€”no explicit preloading needed
- **Automatic cleanup**: No need to call `unloadAsync()`, hooks handle resource cleanup on unmount
- **Multiple players**: Create multiple `useAudioPlayer` instances and store themâ€”all load immediately
