...
- **Separate player and view**: Player logic decoupled from the viewâ€”one player can be used across multiple views
...
- **Android: currentTime in setup**: Setting `player.currentTime` in the `useVideoPlayer` setup callback may not work on Androidâ€”set it after mount instead
