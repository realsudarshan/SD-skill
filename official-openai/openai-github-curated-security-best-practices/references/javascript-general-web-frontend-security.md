...
2. **Security review / vulnerability hunting** in existing frontend code (passive â€œnotice issues while workingâ€ and active â€œscan the repo and report findingsâ€).
...
* MUST NOT request, output, log, hard-code, or commit secrets (API keys intended to be secret, private keys, passwords, OAuth refresh tokens, session tokens, cookies).
  ...
  * If the project uses â€œpublicâ€ keys (e.g., publishable analytics keys), they MUST be treated as non-secret and scoped accordingly.

* MUST NOT â€œfixâ€ security by disabling protections (e.g., weakening CSP with `unsafe-inline`/`unsafe-eval` without justification, removing origin checks for `postMessage`, switching to `innerHTML` for convenience, accepting arbitrary redirects/URLs, or turning off sanitization).
...
* MUST treat uncertainty honestly:
  ...
  * Security headers (CSP, frame-ancestors, etc.) might be set by server/edge/CDN rather than in repo code. If not visible, report as â€œnot visible here; verify at runtime/edge config.â€ (Also note that `<meta http-equiv=...>` only simulates a subset of headers; donâ€™t assume other security headers exist just because a meta tag exists.) ([MDN Web Docs][1])
...
* Browser storage: `localStorage`, `sessionStorage`, IndexedDB (contents can be attacker-influenced via XSS or local machine access; never treat as â€œtrustedâ€). ([OWASP Cheat Sheet Series][6])
* Any data returned from network calls (even if from â€œyour APIâ€), because it may contain stored attacker content that becomes dangerous only when inserted into the DOM. ([OWASP Cheat Sheet Series][2])
...
This is the smallest baseline that prevents common frontend JS/TS security misconfigurations. Some items are â€œin repoâ€ (HTML/JS) and some may live at the server/edge.
...
* If using CSP via `<meta http-equiv>`, MUST understand the limitations:
  ...
  * â€œReport-onlyâ€ CSP cannot be set via a meta element. ([W3C][3])

Practical baseline goals:

* Avoid script sources `unsafe-inline` and `unsafe-eval` (they significantly weaken CSPâ€™s value against XSS). ([MDN Web Docs][10])
...
* SHOULD minimize third-party script execution and treat it as equivalent privilege to first-party JS (it runs with your originâ€™s privileges). ([OWASP Cheat Sheet Series][7])
...
* For â€œrich textâ€ requirements:

  * Sanitize using an allowlist-based sanitizer.
  * Prefer returning safe â€œcomponentsâ€ instead of arbitrary HTML strings.
...
* MUST NOT â€œfix CSP breakageâ€ by adding `unsafe-eval` unless there is a documented, reviewed justification and compensating controls. ([MDN Web Docs][10])
...
* MUST NOT use `setAttribute("onâ€¦", string)` or similar patterns with untrusted data; this coerces strings into executable code in the event-handler context. ([OWASP Cheat Sheet Series][2])
...
* MUST avoid adding `unsafe-inline` as a â€œquick fixâ€ for CSP issues unless explicitly required and reviewed (it defeats much of CSPâ€™s purpose). ([MDN Web Docs][10])
...
* â€œTrusted Types enabledâ€ but policy simply returns input unchanged (no sanitization/validation).
...
* Assuming `localStorage` is â€œtrusted because same-origin.â€
...
* MUST update SRI hashes whenever the underlying resource changes (pin versions; avoid â€œlatestâ€ URLs).
...
* MDN: `frame-ancestors` (and note itâ€™s not supported in `<meta>`): `https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/frame-ancestors` ([MDN Web Docs][18])
