...
- default mode replaces matches with a fixed-length mask (`â–ˆ` repeated), so line breaks and pagination drift less

## Golden path
1. Create a copy of the input DOCX (donâ€™t mutate the only copy).
...
- **Non-text content:** images of text, embedded objects, charts, and tracked changes deletions may contain sensitive info. Masking text nodes wonâ€™t remove those.
