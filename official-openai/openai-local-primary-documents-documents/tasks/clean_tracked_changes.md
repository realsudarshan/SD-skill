...
- `w:ins` â€” inserted content
- `w:del` â€” deleted content
- sometimes: `w:moveTo` / `w:moveFrom` for moved text

Many â€œlooks wrongâ€ reports come from:
- leaving revisions in place (Word shows redlines; LO export may hide/show inconsistently)
- stale renders (you didnâ€™t re-render after patching)
...
2. **Accept** all tracked changes into a clean copy:
   ...
   ```
3. **Render â†’ PNG review** (required):
   ```bash
...
- If Word-specific revision constructs remain, fall back to â€œOpen in Word â†’ Accept All â†’ Save Asâ€ and re-render.
