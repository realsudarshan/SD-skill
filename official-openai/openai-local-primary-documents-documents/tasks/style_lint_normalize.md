...
- `scripts/style_lint.py` â€” report likely inconsistencies
- `scripts/style_normalize.py` â€” conservative cleanup (optional)

## The reliable workflow
1. **Render to PNGs** (baseline) and inspect a few problem areas.
2. Run the **lint** to see what is causing drift.
3. Apply **normalization** only if it matches the userâ€™s intent.
...
```
What to look for:
- Lots of `run_direct_formatting`: common cause of â€œwhy is this one differentâ€.
- Multiple fonts/sizes in `Normal` body text.
- â€œHeading-likeâ€ paragraphs that are not actually Heading styles.
...
## Pitfalls / gotchas
- **Clearing run overrides can remove intentional emphasis.** If the user wants to keep bold/italic emphasis, donâ€™t normalize globally; instead normalize only certain styles/sections.
