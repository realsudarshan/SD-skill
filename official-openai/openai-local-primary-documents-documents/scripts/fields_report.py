...
- A quick "stale field" hint (e.g., TOC present â†’ remind to update fields)
...
def extract_field_instructions(xml_bytes: bytes) -> list[str]:
    ...
    # 2) Complex fields
    # Approach: walk runs in order, track state when we see fldChar begin â†’ gather instrText
    in_field = False
    buf: list[str] = []
...
def main() -> None:
    ...
    if needs_update:
        print("\nREMINDER")
        print(
            "- If the PDF/PNGs show placeholders or wrong page numbers/TOC, open in Word and run: Ctrl+A â†’ F9 (Update Fields), then re-render."
        )
