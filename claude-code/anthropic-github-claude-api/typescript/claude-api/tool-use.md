...
const getWeather = betaZodTool({
  ...
  run: async (input) => {
    ...
    return `72Â°F and sunny in ${input.location}`;
  },
});
...
while (true) {
  ...
  // finalMessage() resolves with the complete Message â€” no need to
...
> **Important:** Don't wrap `.on()` events in `new Promise()` to collect the final message â€” use `stream.finalMessage()` instead. The SDK handles all error/abort/completion states internally.

> **Error handling in the loop:** Use the SDK's typed exceptions (e.g., `Anthropic.RateLimitError`, `Anthropic.APIError`) â€” see [Error Handling](./README.md#error-handling) for examples. Don't check error messages with string matching.
...
Version-suffixed `type` literals; `name` is fixed per interface. Pass plain object literals â€” the `ToolUnion` type is satisfied structurally. **The `name`/`type` pair must match the interface**: mixing `str_replace_based_edit_tool` (20250728 name) with `text_editor_20250124` (which expects `str_replace_editor`) is a TS2322.

**Don't type-annotate as `Tool[]`** â€” `Tool` is just the custom-tool variant. Let structural typing infer from the `tools` param, or annotate as `Anthropic.Messages.ToolUnion[]` if you must:

```typescript
// âœ“ let inference work â€” no annotation
const response = await client.messages.create({
  model: "claude-opus-4-7",
  max_tokens: 16000,
  tools: [
...
// âœ— this is a TS2352 â€” Tool is the CUSTOM tool variant only
...
**Don't mix beta and non-beta types**: if you call `client.beta.messages.create()`, the response `content` is `BetaContentBlock[]` â€” you cannot pass that to a non-beta `ContentBlockParam[]` without narrowing each element.
...
// Reuse container
// container is nullable â€” set only when using server-side code execution
...
### JSON Outputs (Zod â€” Recommended)
...
// parsed_output is null if parsing failed â€” assert or guard
