...
# ToolUseBlock, ...). .type is a Symbol â€” compare with :text, not "text".
...
class GetWeather < Anthropic::BaseTool
  ...
  def call(input)
    "The weather in #{input.location} is sunny and 72Â°F."
  end
end
...
`system_:` (trailing underscore â€” avoids shadowing `Kernel#system`) takes an array of text blocks; set `cache_control` on the last block. Plain hashes work via the `OrHash` type alias. For placement patterns and the silent-invalidator audit checklist, see `shared/prompt-caching.md`.
