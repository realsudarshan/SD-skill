# Prompt Caching â€” Design & Optimization
...
The cache key is derived from the exact bytes of the rendered prompt up to each `cache_control` breakpoint. A single byte difference at position N â€” a timestamp, a reordered JSON key, a different tool in the list â€” invalidates the cache for all breakpoints at positions â‰¥ N.

Render order is: `tools` â†’ `system` â†’ `messages`. A breakpoint on the last system block caches both tools and system together.
...
2. **Classify each input by stability:**
   - Never changes â†’ belongs early in the prompt, before any breakpoint
   - Changes per-session â†’ belongs after the global prefix, cache per-session
   - Changes per-turn â†’ belongs at the end, after the last breakpoint
   - Changes per-request (timestamps, UUIDs, random IDs) â†’ **eliminate or move to the very end**
...
Put a breakpoint on the last system text block. If there are tools, they render before system â€” the marker on the last system block caches tools + system together.
...
Many requests share a large fixed preamble (few-shot examples, retrieved docs, instructions) but differ in the final question. Put the breakpoint at the end of the **shared** portion, not at the end of the whole prompt â€” otherwise every request writes a distinct cache entry and nothing is ever read.

```json
"messages": [{"role": "user", "content": [
  {"type": "text", "text": "<shared context>", "cache_control": {"type": "ephemeral"}},
  {"type": "text", "text": "<varying question>"}  // no marker â€” differs every time
]}]
```
...
| `uuid4()` / `crypto.randomUUID()` / request IDs early in content | Same â€” every request is unique |
| `json.dumps(d)` without `sort_keys=True` / iterating a `set` | Non-deterministic serialization â†’ prefix bytes differ |
...
- Top-level `cache_control` on `messages.create()` auto-places on the last cacheable block â€” simplest option when you don't need fine-grained placement.
- Minimum cacheable prefix is model-dependent. Shorter prefixes silently won't cache even with a marker â€” no error, just `cache_creation_input_tokens: 0`:
...
**Economics:** Cache reads cost ~0.1Ã— base input price. Cache writes cost **1.25Ã— for 5-minute TTL, 2Ã— for 1-hour TTL**. Break-even depends on TTL: with 5-minute TTL, two requests break even (1.25Ã— + 0.1Ã— = 1.35Ã— vs 2Ã— uncached); with 1-hour TTL, you need at least three requests (2Ã— + 0.2Ã— = 2.2Ã— vs 3Ã— uncached). The 1-hour TTL keeps entries alive across gaps in bursty traffic, but the doubled write cost means it needs more reads to pay off.
...
| Field | Meaning |
|---|---|
| `cache_creation_input_tokens` | Tokens written to cache this request (you paid the ~1.25Ã— write premium) |
| `cache_read_input_tokens` | Tokens served from cache this request (you paid ~0.1Ã—) |
| `input_tokens` | Tokens processed at full price (not cached) |

If `cache_read_input_tokens` is zero across repeated requests with identical prefixes, a silent invalidator is at work â€” diff the rendered prompt bytes between two requests to find it.

**`input_tokens` is the uncached remainder only.** Total prompt size = `input_tokens + cache_creation_input_tokens + cache_read_input_tokens`. If your agent ran for hours but `input_tokens` shows 4K, the rest was served from cache â€” check the sum, not the single field.
...
|---|:---:|:---:|:---:|
| Tool definitions (add/remove/reorder) | âŒ | âŒ | âŒ |
| Model switch | âŒ | âŒ | âŒ |
| `speed`, web-search, citations toggle | âœ… | âŒ | âŒ |
| System prompt content | âœ… | âŒ | âŒ |
| `tool_choice`, images, `thinking` enable/disable | âœ… | âœ… | âŒ |
| Message content | âœ… | âœ… | âŒ |

Implication: you can change `tool_choice` per-request or toggle `thinking` without losing the tools+system cache. Don't over-worry about these â€” only tool-definition and model changes force a full rebuild.
...
A cache entry becomes readable only after the first response **begins streaming**. N parallel requests with identical prefixes all pay full price â€” none can read what the others are still writing.

For fan-out patterns: send 1 request, await the first streamed token (not the full response), then fire the remaining Nâˆ’1. They'll read the cache the first one just wrote.
