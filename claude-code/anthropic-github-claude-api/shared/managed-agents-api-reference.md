# Managed Agents â€” Endpoint Reference
...
- Agents and Session Threads have **no delete** â€” only `archive`. Archive is **permanent**: the agent becomes read-only, new sessions cannot reference it, and there is no unarchive. Confirm with the user before archiving a production agent. Environments, Sessions, Vaults, Credentials, and Memory Stores have both `delete` and `archive`; Session Resources, Files, Skills, and Memories are `delete`-only; Memory Versions have neither â€” only `redact`.
- Session resources use `add` (not `create`).
- Go's event stream is `StreamEvents` (not `Stream`).

**Agent shorthand:** `agent` on session create accepts either a bare string (`agent="agent_abc123"` â€” uses latest version) or the full reference object (`{type: "agent", id: "agent_abc123", version: 123}`).

**Model shorthand:** `model` on agent create accepts either a bare string (`model="claude-opus-4-7"` â€” uses `standard` speed) or the full config object (`{type: "model_config", id: "claude-opus-4-6", speed: "fast"}`). Note: `speed: "fast"` is only supported on Opus 4.6.

---

## Agents

**Step one of every flow.** Sessions require a pre-created agent â€” there is no inline agent config under `managed-agents-2026-04-01`.
...
| `POST` | `/v1/agents/{agent_id}/archive` | ArchiveAgent | Archive an agent. Makes it **read-only**; existing sessions continue, new sessions cannot reference it. No unarchive â€” this is the terminal state. |
...
| `POST`   | `/v1/environments/{environment_id}/archive`            | ArchiveEnvironment   | Archive environment. Makes it **read-only**; existing sessions continue, new sessions cannot reference it. No unarchive â€” this is the terminal state. |

## Vaults

Vaults store MCP credentials that Anthropic manages on your behalf â€” OAuth credentials with auto-refresh, or static bearer tokens. Attach to sessions via `vault_ids`. See `managed-agents-tools.md` Â§Vaults for the conceptual guide and credential shapes.
...
Individual text documents inside a store (â‰¤ 100KB each). `create` creates at a `path` and returns `409` (`memory_path_conflict_error`, with `conflicting_memory_id`) if the path is occupied; `update` mutates by `mem_...` ID (rename and/or content). Only `update` accepts a `precondition` (`{"type": "content_sha256", "content_sha256": ...}`) â€” on mismatch returns `409` (`memory_precondition_failed_error`). List endpoints accept `view: "basic"|"full"` (controls whether `content` is populated; `retrieve` defaults to `full`).
...
Immutable per-mutation snapshots (`memver_...`) â€” the audit and rollback surface. `operation` âˆˆ `created` / `modified` / `deleted`.
...
**Always start here.** `model`, `system`, `tools`, `mcp_servers`, `skills` are top-level fields on this object â€” they do NOT go on the session.

```json
{
  "name": "string (required, 1-256 chars)",
  "model": "claude-opus-4-7 (required â€” bare string, or {id, speed} object)",
  ...
  "metadata": {
    "key": "value (max 16 pairs, keys â‰¤64 chars, values â‰¤512 chars)"
  }
}
```

> Limits: `tools` max 128, `skills` max 20, `mcp_servers` max 20 (unique names). `multiagent.agents` 1â€“20 entries (string ID | `{type:"agent",id,version?}` | `{type:"self"}`) â€” see `shared/managed-agents-multiagent.md`.

### CreateSession Request Body

```json
{
  "agent": "agent_abc123 (required â€” string shorthand for latest version, or {type: \"agent\", id, version} object)",
  ...
  "resources": [
    {
      ...
      "mount_path": "/workspace/repo (optional â€” defaults to /workspace/<repo-name>)",
      "checkout": { "type": "branch", "name": "main" }
    }
  ],
  "vault_ids": ["vlt_abc123 (optional â€” MCP credentials with auto-refresh)"],
  "metadata": {
    "key": "value"
  }
}
...
> The `agent` field accepts only a string ID or `{type: "agent", id, version}` â€” `model`/`system`/`tools` live on the agent, not here.
>
...
```json
{
  ...
  "config": {
    "type": "cloud",
    "networking": {
      "type": "unrestricted | limited (union â€” see SDK types)"
    },
    "packages": { }
  },
  "metadata": { "key": "value" }
...
Include the `request_id` when reporting issues to Anthropic â€” it lets us trace the request end-to-end. The inner `error.type` is one of the following:
...
| 429 | `rate_limit_error` | Too many requests â€” check rate limit headers for retry timing |
| 500 | `api_error` | An internal server error occurred |
| 529 | `overloaded_error` | The service is temporarily overloaded â€” retry with backoff |
...
|---|---|---|---|
| Create operations (Agents, Sessions, Vaults) | organization | 60 | â€” |
| All other operations (Agents, Sessions, Vaults) | organization | 600 | â€” |
