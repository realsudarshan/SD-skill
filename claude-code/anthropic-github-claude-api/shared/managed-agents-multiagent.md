# Managed Agents â€” Multiagent Sessions

A coordinator agent can delegate to other agents within one session. All agents **share the container and filesystem**; each runs in its own **thread** â€” a context-isolated event stream with its own conversation history, model, system prompt, tools, MCP servers, and skills (from that agent's own config). Threads are persistent: the coordinator can send a follow-up to a subagent it called earlier and that subagent retains its prior turns.
...
`multiagent` is a **top-level field** on `agents.create()` / `agents.update()` â€” **not** a `tools[]` entry. `agents` lists 1â€“20 roster entries. Nothing changes on `sessions.create()` â€” the roster is resolved from the coordinator's config.
...
orchestrator = client.beta.agents.create(
    ...
    multiagent={
        "type": "coordinator",
        "agents": [
            reviewer.id,                                            # bare string â€” latest version
...
Up to **20 unique agents** in the roster; the coordinator may spawn **multiple copies** of each. **One level of delegation only** â€” depth > 1 is ignored.

---

## Threads

The session-level event stream is the **primary thread** â€” it shows the coordinator's trace plus a condensed view of subagent activity (thread status transitions and cross-thread messages, not every subagent tool call). Drill into a specific subagent via the per-thread endpoints:
...
When a subagent needs your client (an `always_ask` confirmation, or a custom tool result), the request is **cross-posted to the primary thread** with `session_thread_id` identifying the originating thread â€” so you only need to watch the session stream. Reply with `user.tool_confirmation` (carrying `tool_use_id`) or `user.custom_tool_result` (carrying `custom_tool_use_id`), and **echo the `session_thread_id` from the originating event** (the SDK param type and docstring expect it). The server also routes by the tool-use ID, so the echo is belt-and-suspenders rather than load-bearing â€” but include it.
...
- **Depth > 1 is ignored.** A subagent's own `multiagent` roster (if any) doesn't cascade â€” only the session's coordinator delegates.
