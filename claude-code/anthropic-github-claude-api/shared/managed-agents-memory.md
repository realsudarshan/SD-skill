# Managed Agents â€” Memory Stores
...
| Memory | `mem_...` | Store | One text file, addressed by `path` (â‰¤ 100KB each â€” prefer many small files) |
| Memory version | `memver_...` | Memory | Immutable snapshot per mutation; `operation` âˆˆ `created` / `modified` / `deleted` |

## Create a store

`description` is passed to the agent so it knows what the store contains â€” write it for the model, not for humans.
...
Other SDKs: TypeScript `client.beta.memoryStores.create({...})`; Go `client.Beta.MemoryStores.New(ctx, ...)`. See `shared/managed-agents-api-reference.md` â†’ SDK Method Reference for the full per-language table.

Stores support `retrieve` / `update` / `list` (with `include_archived`, `created_at_{gte,lte}` filters) / `delete` / **`archive`**. Archive makes the store read-only â€” existing session attachments continue, new sessions cannot reference it; no unarchive.
...
Memory stores go in the session's `resources[]` array alongside `file` and `github_repository` resources (see `shared/managed-agents-environments.md` â†’ Resources). Memory stores attach at **session create time only** â€” `sessions.resources.add()` does not accept `memory_store`.
...
| --- | --- | --- |
| `type` | âœ… | `"memory_store"` |
| `memory_store_id` | âœ… | `memstore_...` |
| `access` | â€” | `"read_write"` (default) or `"read_only"` â€” enforced at the filesystem level on the mount |
| `instructions` | â€” | Session-specific guidance for this store, in addition to the store's `name`/`description`. â‰¤ 4,096 chars. |

**Max 8 memory stores per session.** Attach multiple when different slices of memory have different owners or lifecycles â€” e.g. one read-only shared-reference store plus one read-write per-user store, or one store per end-user/team/project sharing a single agent config.

### How the agent sees it (FUSE mount)

Each attached store is mounted in the session container at `/mnt/memory/<store-name>/`. The agent interacts with it using the standard file tools (`bash`, `read`, `write`, `edit`, `glob`, `grep`) â€” there are no dedicated memory tools. `access: "read_only"` makes the mount read-only at the filesystem level; `"read_write"` allows the agent to create, edit, and delete files under it. A short description of each mount (name, path, `instructions`, access) is automatically injected into the system prompt so the agent knows the store exists without you having to mention it.
...
`memories.update` accepts a `precondition` so you can read â†’ modify â†’ write back without clobbering a concurrent writer. The only supported type is `content_sha256`. On mismatch the API returns `409` (`memory_precondition_failed_error`) â€” re-read and retry against fresh state.
...
## Audit and rollback â€” memory versions
...
Each version also records `created_by` â€” an actor object with `type` âˆˆ `session_actor` / `api_actor` / `user_actor` â€” and, after redaction, `redacted_at` + `redacted_by`.
...
See `shared/managed-agents-api-reference.md` â†’ Memory Stores / Memories / Memory Versions for the full HTTP method/path tables. Raw HTTP base path:
...
For cURL examples and the CLI (`ant beta:memory-stores ...`), WebFetch the Memory URL in `shared/live-sources.md` â†’ Managed Agents.
