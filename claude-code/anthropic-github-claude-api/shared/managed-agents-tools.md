...
| **Custom tools** | **You** â€” your application handles the call and returns results | Agent emits a `agent.custom_tool_use` event, session goes `idle`, you send back a `user.custom_tool_result` event. |
...
|---|---|---|
| `type` | âœ… | `"agent_toolset_20260401"` |
| `default_config` | âŒ | Applied to all tools. `{ "enabled": bool, "permission_policy": {...} }` |
| `configs` | âŒ | Per-tool overrides: `[{ "name": "...", "enabled": bool, "permission_policy": {...} }]` |
...
1. Agent decides to use the tool â†’ session emits a `agent.custom_tool_use` event with inputs
...
No permission policy needed â€” you're the one executing.
...
1. **Agent creation** declares which servers to connect to (`type`, `name`, `url` â€” no auth). The agent's `mcp_servers` array has no auth field.
...
**Agent side â€” declare servers (no auth):**

| Field | Required | Description |
|---|---|---|
| `type` | âœ… | `"url"` |
| `name` | âœ… | Unique name â€” referenced by `mcp_toolset.mcp_server_name` |
| `url` | âœ… | The MCP server's endpoint URL (Streamable HTTP transport) |
...
> ðŸ’¡ **Per-tool enablement (empirical):** `mcp_toolset` has been observed accepting `default_config: {enabled: false}` + `configs: [{name, enabled: true}]` for an allowlist pattern. The API ref shows only the minimal `{type, mcp_server_name}` form.

> âš ï¸ **MCP auth tokens â‰  REST API tokens.** Hosted MCP servers (`mcp.notion.com`, `mcp.linear.app`, etc.) typically require **OAuth bearer tokens**, not the service's native API keys. A Notion `ntn_` integration token authenticates against Notion's REST API but will **not** work as a vault credential for the Notion MCP server. These are different auth systems.

### Vaults â€” the MCP credential store
...
- **Prefer an MCP server** for that service if one exists â€” it gets the same vault-backed injection.
- **Otherwise, register a custom tool:** the agent emits `agent.custom_tool_use`, your orchestrator (which already holds the credential) executes the call and returns `user.custom_tool_result` over the same authenticated event stream. No public endpoint is exposed; the sandbox never sees the secret. See `shared/managed-agents-client-patterns.md` â†’ Pattern 9.

**Do not put API keys in the system prompt or user messages as a workaround** â€” they persist in the session's event history.

> Formerly known internally as TATs (Tool/Tenant Access Tokens).

**Flow:**

1. Create a vault (`client.beta.vaults.create(...)`) â€” one per tenant/user, or one shared, depending on your model
2. Add MCP credentials to it (`client.beta.vaults.credentials.create(...)`) â€” each credential is tied to one MCP server URL
...
The `refresh` block is what enables auto-refresh â€” `token_endpoint` is where Anthropic posts the `refresh_token` grant. `token_endpoint_auth` is a discriminated union:
...
Omit `refresh` entirely if you only have an access token with no refresh capability â€” it'll work until it expires, then the agent loses access.

> ðŸ’¡ **Getting an OAuth token.** How you obtain the initial access and refresh tokens depends on the MCP server â€” consult its documentation. Once you have them, store them in a vault credential using the shape above; Anthropic auto-refreshes via the `refresh.token_endpoint` from there.

**Scoping:** Vaults are workspace-scoped. Anyone with developer+ role in the API workspace can create, read (metadata only â€” secrets are write-only), and attach vaults. `vault_ids` can be set at session **create** time but not via session update (the SDK docstring says "Not yet supported; requests setting this field are rejected").
...
Two types â€” both work the same way; the agent automatically uses them when relevant to the task at hand:
...
| `version` | â€” | `"latest"` or a specific version number |
