# Managed Agents â€” Common Client Patterns

Patterns you'll write on the client side when driving a Managed Agent session, grounded in working SDK examples.

Code samples are TypeScript â€” Python and cURL follow the same shape; see `python/managed-agents/README.md` and `curl/managed-agents.md` for equivalents.
...
// Tail the live stream. Dedupe only gates handle() â€” terminal checks must run
...
Use this to drive pending â†’ acknowledged UI state for anything you send. How you map a locally-rendered optimistic message to the server-assigned `event.id` is application-specific (typically via the return value of `events.send()` or FIFO ordering).
...
// Drain until the session is truly done â€” see Pattern 5 for the full gate.
for await (const event of stream) {
  if (event.type === 'session.status_terminated') break
  if (
...
Reference: `interrupt.ts` â€” sends the interrupt the moment it sees `span.model_request_start`, drains to idle, then verifies via `sessions.retrieve()`.
  ...
  if (event.type === 'agent.tool_use' && event.evaluated_permission === 'ask') {
    await client.beta.sessions.events.send(session.id, {
      events: [{
        ...
        tool_use_id: event.id,         // not a toolu_ id â€” use event.id
...
- `result` is `'allow' | 'deny'`. Use `deny_message` to tell the model *why* you denied â€” it gets surfaced back to the agent.
...
Do not break on `session.status_idle` alone. The session goes idle transiently â€” e.g. between parallel tool executions, while waiting for a `user.tool_confirmation`, or while awaiting a `user.custom_tool_result`. Break when idle with a terminal `stop_reason`, or on `session.status_terminated`.
...
for await (const event of stream) {
  ...
  if (event.type === 'session.status_idle') {
    if (event.stop_reason.type === 'requires_action') continue // waiting on you â€” handle it
    break // end_turn or retries_exhausted â€” both terminal
  }
}
```

`stop_reason.type` values on `session.status_idle`:
- `requires_action` â€” agent is waiting on a client-side event (tool confirmation, custom tool result). Handle it, don't break.
- `retries_exhausted` â€” terminal failure. Break, then check `sessions.retrieve()` for the error state.
- `end_turn` â€” normal completion.
...
} // else: still running after 2s â€” don't archive, let it settle or escalate
```
...
The `Promise.all([stream, send])` shape works too, but stream-first is simpler and has the same effect â€” the stream starts buffering the moment it's opened.
...
// uploaded.id         â†’ the original file
const session = await client.beta.sessions.create({
  /* ... */
  resources: [{ type: 'file', file_id: uploaded.id, mount_path: '/workspace/data.csv' }],
})
// session.resources[0].file_id !== uploaded.id  â† different IDs
```

Delete the original via `files.delete(uploaded.id)`; the session-scoped copy is garbage-collected with the session. `mount_path` must be absolute â€” see `shared/managed-agents-environments.md`.

---

## 9. Secrets for non-MCP APIs and CLIs â€” keep them host-side via custom tools

**Problem:** you want the agent to call a third-party API or run a CLI that needs a secret (API key, token, service-account credential), but there is currently no way to set environment variables inside the session container, and vaults currently hold MCP credentials only â€” they are not exposed to the container's shell. So `curl`, installed CLIs, or SDK clients running via the `bash` tool have no first-class place to read a secret from.
...
**Security note:** this does not expose a public endpoint. `agent.custom_tool_use` arrives on the SSE stream your orchestrator already holds open with your Anthropic API key, and `user.custom_tool_result` goes back via `events.send()` under the same key. Your orchestrator is a client, not a server â€” nothing unauthenticated is listening.

**Do not embed API keys in the system prompt or user messages as a workaround.** Prompts and messages are stored in the session's event history, returned by `events.list()`, and included in compaction summaries â€” a secret placed there is durably persisted and readable via the API for the life of the session.
