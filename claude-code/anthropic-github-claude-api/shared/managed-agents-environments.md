...
|---|---|---|
| `type` | âœ… | `"github_repository"` |
| `url` | âœ… | The GitHub repository URL |
| `authorization_token` | âœ… | GitHub Personal Access Token with repository access. **Never echoed in API responses.** |
| `mount_path` | âŒ | Path where the repository will be cloned. Defaults to `/workspace/<repo-name>`. |
| `checkout` | âŒ | `{type: "branch", name: "..."}` or `{type: "commit", sha: "..."}`. Defaults to the repo's default branch. |
