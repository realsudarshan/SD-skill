...
// Explicit API key (use environment variables â€” never hardcode keys)
AnthropicClient client = new() {
...
**`RawMessageStreamEvent` TryPick methods** (naming drops the `Message`/`Raw` prefix): `TryPickStart`, `TryPickDelta`, `TryPickStop`, `TryPickContentBlockStart`, `TryPickContentBlockDelta`, `TryPickContentBlockStop`. There is no `TryPickMessageStop` â€” use `TryPickStop`.
...
var response = await client.Messages.Create(new MessageCreateParams
{
    Model = Model.ClaudeOpus4_6,
    MaxTokens = 16000,
    // ThinkingConfigParam? implicitly converts from the concrete variant classes â€”
    // no wrapper needed.
...
`Tool` (NOT `ToolParam`) with an `InputSchema` record. `InputSchema.Type` is auto-set to `"object"` by the constructor â€” don't set it. `ToolUnion` has an implicit conversion from `Tool`, triggered by the collection expression `[...]`.
...
// No .ToParam() â€” reconstruct per variant. Implicit conversions from each
...
foreach (ContentBlock block in response.Content)
{
    ...
    {
        // Signature MUST be preserved â€” the API rejects tampering
    ...
    {
        // ToolUseBlock has required Caller; ToolUseBlockParam.Caller is optional â€” don't copy it
        ...
        // Execute the tool; collect ONE result per tool_use block â€” the API
...
`ToolResultBlockParam` has no tuple constructor â€” use the object initializer. `Content` is a string-or-list union; a plain `string` implicitly converts.
...
`BetaMessage.Content` is `IReadOnlyList<BetaContentBlock>` â€” a 15-variant discriminated union. Narrow with `TryPick*`. **Response `BetaContentBlock` is NOT assignable to param `BetaContentBlockParam`** â€” there's no `.ToParam()` in C#. Round-trip by converting each block:

```csharp
using Anthropic.Models.Beta.Messages;

var betaParams = new MessageCreateParams   // no Beta prefix â€” one of only 2 unprefixed
{
    Model = Model.ClaudeOpus4_6,
    MaxTokens = 16000,
...
foreach (BetaContentBlock block in resp.Content)
{
    ...
    {
        // Content is nullable â€” compaction can fail server-side
...
**`BetaToolUseBlock.Input` is `IReadOnlyDictionary<string, JsonElement>`** â€” index by key then call the `JsonElement` extractor:
...
`MessageCountTokensParams.Tools` uses a different union type (`MessageCountTokensTool`) than `MessageCreateParams.Tools` (`ToolUnion`) â€” if you're passing tools, the compiler will tell you when it matters.
...
The non-beta `DocumentBlockParamSource` union has no file-ID variant â€” file references need `client.Beta.Messages.Create()`.
